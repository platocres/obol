...
## 4. Land the findings the right way
...
## 6. Release every build

Every build is a versioned release. In the same PR:
...
## 7. Validate and keep it green

- Inner loop: `node tools/scope-check.js`.
- Notes/queue gates: `node tools/validate-notes-impact.js`, `node tools/validate-note-integration.js`, `node tools/validate-note-remining-audits.js`, `node tools/sync-product-build-next.js --check`, `node tools/validate-product-hardening-queue.js`.
