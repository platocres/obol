...
## Release-final contract

The release-final head is authoritative only when the Product Hardening preflight, complete historical contract chain, and browser smoke all pass on that exact commit. Any later source commit invalidates that evidence and requires the release-final gates to run again.
