...
## Single-paint application boot in v9.46

Runtime compaction now tracks three different outcomes separately: fewer requests, fewer live historical semantics, and fewer visible historical paints. v9.43 achieved the first for the application area and retired proven-dead overlays, but the 43 surviving exact-owned fragments still execute in historical order. v9.46 does not pretend those fragments are semantically flattened. Instead, the static entrypoint is hidden during compatibility initialization and the stable current workflow explicitly commits the first visible route. A throttled Chromium test delays the current workflow so any regression would visibly expose historical Home layers. The runtime consolidation projection remains 172 semantically flattened, 100 exact-owned, and 55 retired because visibility ownership is not fragment retirement.

## v9.47 — Semantic application execution ownership

v9.43 and v9.46 are intentionally preserved as narrower milestones. v9.43 reduced the application request layer to 43 load-bearing historical fragments behind one exact owner and retired 21 stale C.VERSION-gated overlays. v9.46 added a boot visibility barrier that proved the first visible Home frame was current. Neither milestone removed the surviving historical route wrappers, timers, hashchange listeners, or delayed decorators from execution.

v9.47 changes the ownership category itself. `assets/obol-app-current.js` is generated by `tools/sync-app-current.js` as a semantic delta replay. The 43 source fragments stay on disk and remain listed as the frozen semantic/equivalence ledger, but their autonomous startup schedulers and navigation listeners are suppressed. One stable current route transaction invokes the surviving rendering semantics, drains the historical decorator queue synchronously, runs stable current workflow/operator renderers last, and owns the single live hashchange listener.

This moves all 43 application fragments from the "still exact-owned" runtime bucket into the semantically flattened bucket without changing the 327-fragment frozen ledger or the 21-fragment v9.43 retirement result. The metric change is additive: v9.43 request retirement, v9.46 first-paint protection, and v9.47 semantic execution ownership are three different completed contracts.

The browser proof intentionally observes beyond the old 5.2-second historical timer horizon and exercises forced reroutes plus Home → Next Steps → Home navigation. A future agent must not replace that with a short first-frame check.

## v9.48 — Evidence overlay chain restoration

v9.44 retired four Evidence overlays (`intake-v7.7`/`v7.8`/`v7.9`/`v8.2`) that could never execute: each hooked a predecessor `OBOL_INTAKE_*` global that never publishes `analyzeTerminal`, so each returned at its guard. That retirement was correct and stays in force — the four files remain retired in the frozen ledger and the retirement proof is unchanged. What v9.44 could not do was ship the conservative Evidence those overlays were written to add; it filed that gap as `cc-evidence-chain-restore`.

v9.48 closes the defect without reviving the dead files. The overlay logic is re-homed verbatim onto the live decorator chain by a stable current owner, `assets/intake-evidence-restore.js`, which hooks the correct global (`OBOL_INTAKE_V21`) instead of a helper-only predecessor. It is a route-lazy current decorator — not a frozen fragment, not part of the exact-concatenation Evidence bundle — loaded after the Evidence bundle on the Evidence/Artifacts routes so it wraps the fully-assembled `analyzeTerminal` last, in the original intended overlay order.

This is a distinct outcome from retirement: v9.44 removed dead code; v9.48 restores missing behavior through a current owner. `tools/validate-evidence-current-equivalence.js` now proves both at once — the retirement is still observably inert, and the restoration actually decorates `analyzeTerminal` (the exact step the broken subchain skipped) and surfaces conservative Evidence across all four families with proof boundaries intact. Future agents must keep the four historical overlays retired: correcting a dead overlay's hook in place would reopen the v9.44 retirement rather than restoring behavior additively.
