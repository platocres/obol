...
**Consolidation is not automatic retirement.** The frozen ledger is untouched. Current accounting is 172 domain/core fragments semantically flattened, 125 fragments still executing through exact owners, and 30 Dashboard fragments retired to fixtures. Product Build Next therefore leads with the next queued flattening item per remaining ownership area — application, Evidence parsing, and stylesheet. Each is a separate pass with its own equivalence, workspace-migration, and test-retirement proof. Do not flatten two areas at once: their migration surfaces are unrelated, and a combined pass cannot be rolled back cleanly.
