...
v9.40 later closed the delivery half that v9.7 deliberately left open by moving the 69-fragment cascade into one exact-concatenation request. v9.45 closes the semantic half: the same stable owner is generated as a conservative cascade snapshot, with superseded declarations removed only under exact selector/property/grouping-context identity and with Chromium visual equivalence against the frozen historical cascade.

## Runtime layer consolidation

v9.40 completes `runtime-area-consolidation`. Every historical runtime ownership area resolves to exactly one stable, non-versioned owner — three at operator startup (domain 103, core 69, application 64) and four route-lazy (Evidence 41, Nmap 3, report overlays 14, tool reference 3) — plus the flattened stylesheet cascade. v9.41 then completes `runtime-domain-flattening`: the domain owner is no longer a live 103-fragment concatenation, but an authored semantic graph snapshot proven equivalent to that frozen ledger. v9.42 completes `runtime-core-flattening`: the core owner is now a generated semantic delta replay that preserves the shared v2 base scope and replays the 69-fragment state/derivation chain without executing those fragments directly in current browser or Node runtime paths. v9.43 completes `runtime-app-flattening` by proving supersession rather than rewriting code: the 21 release-wave overlays whose whole contribution is gated on a stale `C.VERSION` leave live startup, taking the application area from 64 exact-owned fragments to 43.
...
The frozen v9.5 fragment ledger and its order fingerprints are untouched. `tools/sync-domain-current.js` generates the semantic domain owner and `tools/validate-domain-current-equivalence.js` proves the complete graph/function equivalence. `tools/sync-core-current.js` generates the semantic core owner and `tools/validate-core-current-equivalence.js` proves exported roots, `C.*` surface, helper globals, migration helpers, workspace migration/coercion, Evidence application, recommendation ranking, report readiness, project-model, search, network, sanitized-export, and Nmap-builder behavior. `tools/sync-runtime-bundles.js` still generates the five exact owners; `tools/validate-runtime-bundles.js` proves their exact-concatenation hazards and observable equivalence. `tests/playwright-smoke.js` enforces a per-route request ceiling and fails when a historical fragment is fetched directly.

v9.44 then retires four unreachable Evidence overlays, leaving 37 reachable Evidence fragments and 55 total JavaScript fragments retired to the frozen ledger. v9.45 completes the last separate ownership-area pass by replacing the 69-fragment exact CSS concatenation with a semantic cascade snapshot backed by static reduction proof and Chromium visual equivalence. **Runtime Layer Consolidation now has no live items.** Product Build Next advances to the critical Evidence-chain restoration defect uncovered during v9.44 rather than opening another runtime-flattening layer.

The ownership sequence shows that flattening has multiple safe shapes. Domain and core were rewritten into semantic owners. Application and Evidence used *subtraction* where reachability/supersession could be proven. CSS uses a narrow semantic cascade reduction and an independent browser comparison. Prefer subtraction where it is available, and require equivalence proof whenever current behavior is re-authored.
