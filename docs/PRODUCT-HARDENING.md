...
The frozen v9.5 fragment ledger and its order fingerprints are untouched. `tools/sync-domain-current.js` generates the semantic domain owner and `tools/validate-domain-current-equivalence.js` proves the complete graph/function equivalence. `tools/sync-runtime-bundles.js` still generates the six exact owners; `tools/validate-runtime-bundles.js` proves their exact-concatenation hazards and core observable equivalence. `tests/playwright-smoke.js` enforces a per-route request ceiling and fails when a historical fragment is fetched directly.

Current runtime accounting now distinguishes 103 semantically flattened domain fragments, 194 fragments still executing through exact current owners, and 30 Dashboard fragments retired to the frozen ledger. Product Build Next therefore keeps the **Runtime Layer Consolidation** package active for the remaining ownership areas — core, application, Evidence parsing, and stylesheet — each with its own equivalence, workspace-migration, and test-retirement proof. Do not flatten two areas in one pass.
