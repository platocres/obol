...
Current runtime accounting now distinguishes 172 semantically flattened domain/core fragments, 125 fragments still executing through exact current owners, and 30 Dashboard fragments retired to the frozen ledger. Product Build Next therefore keeps the **Runtime Layer Consolidation** package active for the remaining ownership areas — application, Evidence parsing, and stylesheet — each with its own equivalence, workspace-migration, and test-retirement proof. Do not flatten two areas in one pass.
