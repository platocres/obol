...
The v9.84 contract explicitly checks `OBOL_PRODUCT_HARDENING.nextNotesBatch`, `concreteBuildNext(1)`, and `OBOL_SOURCE_NOTE_CLUSTERS.reviewQueue[0]` so the README and Product Hardening Dashboard do not drift apart. Browser smoke keeps the dashboard route inside the current request budget after the v9.84 release extension is added.
