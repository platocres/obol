...
The shared plumbing helper in `data/product-hardening/tool-builder-shared-plumbing-current.js` owns repeated mechanics for acceleration slices: schema registration, duplicate-safe install, inventory disposition patching, Evidence profile wrapping, intake wrapping, redaction, conservative state selection, Tools route refresh, and audit-queue refresh. Shared code owns mechanics; tool profiles own judgment.

The credential-helper current owner covers CeWL, crunch, hashid, and name-that-hash. The privilege-helper current owner covers pspy, accesschk, and searchsploit. The remote-exec current owner covers Impacket PsExec, WMIExec, SMBExec, DCOMExec, and ATExec. The database current owner covers mysql, psql, redis-cli, odat, and impacket-mssqlclient, with the generic `mssql` inventory record routing to the Impacket MSSQL client builder. These are complete-through metadata only; they do not close the full modeled-tool backlog.
...
- **Remote execution and lateral movement:** remaining runas/cmdkey, Windows service/task helpers, RDP/VNC clients, PowerShell/cmd launchers, MSSQL execution follow-through not covered by the database owner, and remote-exec utilities not already covered by the Impacket PsExec/WMIExec/SMBExec/DCOMExec/ATExec current owner.
...
- **Cloud, container, database, and service-specific tooling:** aws, awslocal, kubectl, docker/lxc, IPMI tooling, database-adjacent clients or helpers not already covered by mysql, psql, redis-cli, odat, or impacket-mssqlclient, and service-specific enumeration/abuse utilities.
