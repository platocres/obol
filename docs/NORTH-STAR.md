...
v8.7 preserves the completed canonical and frozen-baseline milestones while advancing whole-file source inventory:

- canonical: 127 / 127 implemented, 0 partial, 0 gaps, 0 stale;
- strict completion: 100%;
- represented coverage: 100%;
- methodology source files atomized: 16 / 17;
- frozen partial baselines decomposed: 34 / 34;
- currently inventoried atomic units fidelity-complete: 316 / 316;
- remaining inventoried atomic audits: 0;
- remaining whole-file source-inventory items: 1.

v8.7 makes no canonical breadth change. It completes `trusts.md` across trust enumeration, domain-SID discovery, child-to-parent and parent-to-child branches, external/forest trust direction, foreign membership, SIDHistory/extra-SID routes, unconstrained-delegation routing, AD CS routing, credential reuse, and MSSQL linked servers.

Thirty-four source units receive complete terminal review. Thirty-three are modeled through mature owners. The legacy MSSQL `trustlink` / `sp_linkedservers` spelling is explicitly superseded by the current linked-server workflow rather than converted into a duplicate operator route.

Source command review remains explicit. The pinned concatenated `lookupsid.py` line is split into separate current Impacket calls. The child-to-parent Golden Ticket SID field is corrected to use the child domain SID. External-trust Impacket ticket routes keep the domain SID distinct from the DNS domain and use the reviewed trusted-domain `krbtgt` SPN. One-way trust constraints remain explicit.

Evidence remains conservative. Trust discovery, SIDs, recovered trust/krbtgt material, forged tickets, foreign membership, credentials, delegation/coercion context, SQL links, certificate paths, cross-domain authentication, service access, execution, administrator/SYSTEM context, Enterprise Admin membership, privilege, and cleanup remain separate proof states. Project/source accounting metadata never creates engagement facts.

The file-level denominator is now 16/17. Therefore 127/127 canonical implementation, 34/34 frozen-baseline decomposition, and 316/316 current atomic completion still do **not** mean Orange source fidelity is globally complete.
...
v8.7 has zero rows in priorities 1 through 5, so the live phase remains priority 6. The final remaining file is `valid_user.md`.

This ordering is derived from the repository model and exposed through the stable `C.currentProjectModel(...)` pointer. The current versioned implementation is `C.projectModel87(...)`.

## Current-project projection rule

Beginning with v6.6, project status has one current projection boundary. v6.8 added the stable non-versioned pointer `C.currentProjectModel(...)` so current consumers do not need to hard-code whichever versioned adapter happens to be newest. v8.7 keeps that pointer current while retaining versioned adapters as historical regression boundaries.
...
- **316/316 currently inventoried atomic units complete** means the present atomic ledger is complete, not that the remaining `valid_user.md` source file has been exhausted.
