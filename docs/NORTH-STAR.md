...
v8.0 completed that frozen 34-row baseline. The current inventory phase is broader: every methodology-bearing Orange source file must itself be atomized before source inventory can be called complete. v8.1 completed `low_access.md`; v8.2 completes `crack_hash.md`.
...
## Current v8.2 baseline

v8.2 preserves the completed canonical and frozen-baseline milestones while advancing whole-file source inventory:

- canonical: 127 / 127 implemented, 0 partial, 0 gaps, 0 stale;
- strict completion: 100%;
- represented coverage: 100%;
- methodology source files atomized: 11 / 17;
- frozen partial baselines decomposed: 34 / 34;
- currently inventoried atomic units fidelity-complete: 226 / 226;
- remaining inventoried atomic audits: 0;
- remaining whole-file source-inventory items: 6.

v8.2 makes no canonical breadth change. It completes the remaining terminal inventory for `crack_hash.md` across LM, NT, NetNTLMv1, NetNTLMv2, Kerberos TGS RC4/AES128, AS-REP, MSCache2, TimeRoast, and the inherited SCCM/PXE branch.

Fifteen new source units receive complete terminal review and the existing v8.0 SCCM/PXE unit remains part of the file-level denominator. Fourteen new units are modeled through mature local cracking owners. The external `crack.sh` alternative is explicitly superseded as the preferred Obol path to avoid making third-party transmission of captured authentication material the default. The pinned NetNTLMv1 Hashcat `-m 1000` line is preserved in source lineage but modeled with the corrected NetNTLMv1 mode 5500.

Evidence remains conservative. Hash identification, hash loading, cracking invocation, candidate-space exhaustion, recovered plaintext, credential validation, authenticated access, execution, administrator/SYSTEM context, and privilege remain separate proof states. An explicit recovered plaintext result creates credential-candidate material only. Project/source accounting metadata never creates engagement facts.

The file-level denominator is now 11/17. Therefore 127/127 canonical implementation, 34/34 frozen-baseline decomposition, and 226/226 current atomic completion still do **not** mean Orange source fidelity is globally complete.
...
v8.2 has zero rows in priorities 1 through 5, so the live phase remains priority 6. The six remaining files are `low_hanging.md`, `persistence.md`, `dom_admin.md`, `know_vuln_auth.md`, `trusts.md`, and `valid_user.md`. `low_hanging.md` is now first.

This ordering is derived from the repository model and exposed through the stable `C.currentProjectModel(...)` pointer. The current versioned implementation is `C.projectModel82(...)`.

## Current-project projection rule

Beginning with v6.6, project status has one current projection boundary. v6.8 added the stable non-versioned pointer `C.currentProjectModel(...)` so current consumers do not need to hard-code whichever versioned adapter happens to be newest. v8.2 keeps that pointer current while retaining versioned adapters as historical regression boundaries.
...
- **226/226 currently inventoried atomic units complete** means the present atomic ledger is complete, not that the remaining six source files have been exhausted.
