...
v8.0 completed that frozen 34-row baseline. The current inventory phase is broader: every methodology-bearing Orange source file must itself be atomized before source inventory can be called complete. v8.1 completed `low_access.md`; v8.2 completed `crack_hash.md`; v8.3 completes `low_hanging.md`.
...
## Current v8.3 baseline

v8.3 preserves the completed canonical and frozen-baseline milestones while advancing whole-file source inventory:
...
- currently inventoried atomic units fidelity-complete: 243 / 243;
- remaining inventoried atomic audits: 0;
- remaining whole-file source-inventory items: 5.

v8.3 makes no canonical breadth change. It completes `low_hanging.md` across EternalBlue, Tomcat/JBoss Manager, Java RMI, Java deserialization, Log4Shell, MSSQL, Exchange ProxyShell, Veeam, GLPI, weak-service scanning, and the inherited Zerologon branch.

Seventeen new source units receive complete terminal review and the two v8.0 Zerologon units remain part of the file-level denominator. Sixteen new units are modeled through mature or focused owners. The pinned Nessus alternative is explicitly superseded as Obol's default broad-scanner path in favor of Nuclei plus targeted validation, while remaining visible in source lineage.

Evidence remains conservative. Product/version exposure, scanner output, module selection, PoC startup, payload generation or delivery, callback behavior, recovered credential material, authenticated access, execution, administrator/SYSTEM context, and privilege remain separate proof states. Project/source accounting metadata never creates engagement facts.

The file-level denominator is now 12/17. Therefore 127/127 canonical implementation, 34/34 frozen-baseline decomposition, and 243/243 current atomic completion still do **not** mean Orange source fidelity is globally complete.
...
v8.3 has zero rows in priorities 1 through 5, so the live phase remains priority 6. The five remaining files are `persistence.md`, `dom_admin.md`, `know_vuln_auth.md`, `trusts.md`, and `valid_user.md`. `persistence.md` is now first.

This ordering is derived from the repository model and exposed through the stable `C.currentProjectModel(...)` pointer. The current versioned implementation is `C.projectModel83(...)`.

## Current-project projection rule

Beginning with v6.6, project status has one current projection boundary. v6.8 added the stable non-versioned pointer `C.currentProjectModel(...)` so current consumers do not need to hard-code whichever versioned adapter happens to be newest. v8.3 keeps that pointer current while retaining versioned adapters as historical regression boundaries.
...
- **243/243 currently inventoried atomic units complete** means the present atomic ledger is complete, not that the remaining five source files have been exhausted.
