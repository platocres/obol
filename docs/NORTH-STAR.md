...
v8.0 completed that frozen 34-row baseline. The current inventory phase is broader: every methodology-bearing Orange source file must itself be atomized before source inventory can be called complete. v8.1 completed `low_access.md`; v8.2 completed `crack_hash.md`; v8.3 completed `low_hanging.md`; v8.4 completes `persistence.md`.
...
v8.4 preserves the completed canonical and frozen-baseline milestones while advancing whole-file source inventory:
...
- currently inventoried atomic units fidelity-complete: 258 / 258;
- remaining inventoried atomic audits: 0;
- remaining whole-file source-inventory items: 4.

v8.4 makes no canonical breadth change. It completes `persistence.md` across Add Domain Admin membership, Golden and Silver Tickets, DSRM, Skeleton Key, Custom SSP, Golden Certificate, Diamond and Sapphire Tickets, DCShadow, ACL manipulation, and the source's trailing placeholder.

Fifteen new source units receive complete terminal review and the existing v8.0 Add Domain Admin unit remains part of the file-level denominator. Fourteen new units are modeled through mature persistence owners. The final source ellipsis is explicitly rejected as non-actionable placeholder text instead of being converted into invented methodology.

Source command review remains explicit. The pinned Golden Ticket Mimikatz line is normalized so `/aes256` is a separate argument. The pinned Silver Ticket source swaps domain/SID semantics and omits the Impacket `-spn` requirement; v8.4 preserves that lineage while modeling the corrected service-scoped workflow. Diamond/Sapphire password, NT hash, and AES-key placeholders are treated as alternative authentication material rather than blindly combined.

Evidence remains conservative. Persistence configuration, generated ticket/certificate material, credential artifacts, authentication, service acceptance, execution, administrator/SYSTEM context, privilege, and cleanup remain separate proof states. Project/source accounting metadata never creates engagement facts.

The file-level denominator is now 13/17. Therefore 127/127 canonical implementation, 34/34 frozen-baseline decomposition, and 258/258 current atomic completion still do **not** mean Orange source fidelity is globally complete.
...
v8.4 has zero rows in priorities 1 through 5, so the live phase remains priority 6. The four remaining files are `dom_admin.md`, `know_vuln_auth.md`, `trusts.md`, and `valid_user.md`. `dom_admin.md` is now first.

This ordering is derived from the repository model and exposed through the stable `C.currentProjectModel(...)` pointer. The current versioned implementation is `C.projectModel84(...)`.

## Current-project projection rule

Beginning with v6.6, project status has one current projection boundary. v6.8 added the stable non-versioned pointer `C.currentProjectModel(...)` so current consumers do not need to hard-code whichever versioned adapter happens to be newest. v8.4 keeps that pointer current while retaining versioned adapters as historical regression boundaries.
...
- **258/258 currently inventoried atomic units complete** means the present atomic ledger is complete, not that the remaining four source files have been exhausted.
