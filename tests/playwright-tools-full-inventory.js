...
async function inventoryProof(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      fullInventoryFlag: !!(window.OBOL_TOOLS_LIBRARY_CURRENT && window.OBOL_TOOLS_LIBRARY_CURRENT.fullInventoryVisible),
      runtimeHiddenKeys: Array.from(window.__OBOL_TOOLS_LIBRARY_HIDDEN_INVENTORY_KEYS__ || []),
      renderedTotal: window.__OBOL_TOOLS_LIBRARY_INVENTORY_TOTAL_COUNT__ || null,
      renderedVisible: window.__OBOL_TOOLS_LIBRARY_INVENTORY_VISIBLE_COUNT__ || null
    };
  });
}
...
async function waitForInventoryCompletion(page) {
  await page.waitForFunction(() => {
    ...
    return hidden.length === 0 && visible.has(key('impacket-psexec')) && /psexec/i.test(document.body && document.body.innerText || '');
  }, null, { timeout: 30000 });
}
...
(async () => {
  ...
  try {
    ...
    try {
      ...
      failures.push('Tools home does not visibly account for every Tool Builder inventory key. Proof: ' + JSON.stringify(proof));
    }

    const proof = await inventoryProof(page);
    if (!proof.inventoryLoaded) failures.push('Tool Builder inventory was not loaded on #/tools');
    if (!proof.fullInventoryFlag) failures.push('Tools library did not publish fullInventoryVisible');
    ...
    if (proof.runtimeHiddenKeys.length) failures.push('Runtime inventory owner reports hidden keys: ' + proof.runtimeHiddenKeys.join(', '));
  ...
  console.log('Tools full-inventory browser proof passed: every Tool Builder inventory key is visible on #/tools, and impacket-psexec is visible and routeable.');
})().catch((err) => {
