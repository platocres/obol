...
assert.throws(()=>runtime.compile(psql,{action:'copyProgramExec',host:'203.0.113.77',port:'5432',username:'postgres',database:'postgres',sslMode:'prefer',osCommand:'id'},{}),/Missing required fields/,'COPY PROGRAM must be risk-gated');
const mssql=schema.get('tb-impacket-mssqlclient');
assert.throws(()=>runtime.compile(mssql,{action:'enableXpCmdshell',target:'203.0.113.40',identityScope:'domain',domain:'CORP',username:'alice',authMode:'password',password:'Secret123',port:'1433',windowsAuth:true},{}),/Missing required fields/,'xp_cmdshell enablement must be risk-gated');
const queue=read('docs/TOOL-BUILDER-BUILD-QUEUE.md');
assert(queue.includes('Implemented Tool Builder operator-guidance audit and repair'),'queue should make operator guidance repair the first Tool Builder batch');
assert(queue.includes('Do not resume new modeled-tool promotion'),'queue should pause new modeled-tool promotion until guidance repair is done');
