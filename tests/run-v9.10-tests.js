...
for(const rel of ['data/current-release.js','data/product-hardening/product-hardening-queue.js','data/product-hardening/item-test-contracts.js','data/field-notes.js'])vm.runInContext(read(rel),sandbox,{filename:rel});
...
assert(release&&q&&contracts&&fieldNotes,'v9.10 release, queue, contracts, and field-notes owners load');
assert.deepStrictEqual([release.version,release.label,release.phase,release.orangeBaseline],['9.10.0','v9.10','product-hardening','v8.8']);

const item=q.items.find(i=>i.id==='ux-progressive-notes');
assert(item,'progressive field-notes item remains in durable queue');
assert.strictEqual(item.status,'complete','progressive field-notes item is complete');
assert.strictEqual(q.tracks.find(t=>t.id==='ui-ux').complete,6,'UI/UX advances to six completed items');
assert.strictEqual(q.buildNext(1)[0].id,'ux-mobile-density','Product Build Next advances to responsive density');
const itemContract=contracts.contracts['ux-progressive-notes'];
assert(itemContract&&itemContract.acceptance.length&&itemContract.validationCommands.length&&itemContract.proofFiles.length,'progressive notes has item-specific Definition of Done');
...
assert(releaseDoc.includes('# Obol v9.10')&&releaseDoc.includes('ux-progressive-notes'),'release documentation owns the v9.10 queue disposition');
assert(uxDoc.includes('contextual field notes')||uxDoc.includes('field notes'),'UX documentation records the field-notes presentation boundary');
...
for(const command of [
 ...
 ['tools/validate-release-pr.js','--repo-only','--release-version=9.10']
]){
