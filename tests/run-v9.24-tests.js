...
state.activities=[{id:'activity-success',cardId:'card-a',result:'success',manualOutcomeId:'mo-success'}];state.facts=[{id:'foothold.linux',source:'manual-outcome:mo-success'},{id:'scan.initial',source:'intake:nmap'}];success.activityId='activity-success';
...
const ui=read('assets/manual-outcomes-current.js');for(const token of ['Mark successful','Mark failed','Mark blocked','Mark skipped','data-cardroot','data-manual-queue-outcome','needs Evidence for report','installReportBoundary','projectReportState'])assert(ui.includes(token),'manual outcome UI contains '+token);assert(ui.includes('[data-mark=\\"tried\\"],[data-mark=\\"success\\"]')||ui.includes('[data-mark="tried"],[data-mark="success"]'),'legacy tried/success controls are intercepted rather than allowed to bypass the boundary');
