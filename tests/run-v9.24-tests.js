...
assert.strictEqual(q.tracks.find(track=>track.id==='manual-outcomes').complete,8);assert.strictEqual(q.totals().complete,61);assert.strictEqual(q.totals().queued,13);assert(q.buildNext(1)[0]&&q.buildNext(1)[0].id==='notes-enex-extraction');assert(packages.recommend(q)&&packages.recommend(q).id==='notes-integration-platform');assert.deepStrictEqual(Array.from(packages.validate(q)),[]);
assert.deepStrictEqual(Array.from(manual.OUTCOMES),['success','failed','blocked','skipped']);for(const reason of ['auth-failed','timeout','no-results','syntax-issue','blocked','not-vulnerable','other'])assert(manual.validateFailureReason(reason));
