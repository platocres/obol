...
assert.strictEqual(q.tracks.find(track=>track.id==='notes-integration').complete,41,'historical v9.27 projection remains forty-one reviewed notes');
assert.strictEqual(q.totals().complete,103,'historical v9.27 shared denominator remains 103/632');
...
assert.strictEqual(field.entries.length,18,'v9.27 retains eighteen public rewritten field notes');
