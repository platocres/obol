...
assert.deepStrictEqual(wave.primaryCardIds,['windows-service-permission-review']);
...
const ownerCards=['windows-service-permission-review'];
const contextCards=['windows-identity-privilege-review','windows-unquoted-service-path-review','windows-scheduled-task-chain-review','windows-alwaysinstall-elevated-review','windows-credential-trail-review','windows-token-privilege-review'];
const owner=globalThis.CARDS&&globalThis.CARDS['windows-service-permission-review'];
assert.ok(owner,'missing folded Windows privesc owner card');
assert.strictEqual(owner.cardKind,'primary');
assert.ok((owner.featureIds||[]).includes('windows-privesc-services-local-admin-router'));
assert.ok((owner.commands||[]).length>=12,'folded owner needs complete Windows-privesc command spine');
...
assert.ok(/Windows|service|task|registry|credential|token|privilege|admin/i.test(owner.operatorGoal+owner.hypothesis+owner.whyNow));
...
for(const id of contextCards){const card=globalThis.CARDS&&globalThis.CARDS[id];assert.ok(card,'context card should still exist '+id);assert.ok(!((card.featureIds||[]).includes('windows-privesc-services-local-admin-router')),'context card should not become another v9.88 primary owner '+id);}
...
for(const note of notes.filter(note=>note&&wave.publicNoteIds.includes(note.id))){assert.deepStrictEqual(note.cardIds,ownerCards,'v9.88 field note should bind to folded owner card');assert.deepStrictEqual(note.pathIds,ownerCards,'v9.88 path note should bind to folded owner card');assert.deepStrictEqual(note.relatedCardIds,contextCards,'v9.88 field note should preserve adjacent context cards');}
...
assert.deepStrictEqual(analysis.cardIds,['windows-service-permission-review']);
...
console.log('v9.88 mined Windows privilege escalation service, registry, scheduled task, credential, token, and local-admin mechanics into one existing owner card and advanced the cluster queue to AD enumeration.');
