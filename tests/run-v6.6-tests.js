...
test('README remains compact and keeps permanent project contracts',()=>{const readme=fs.readFileSync(path.join(root,'README.md'),'utf8'),size=Buffer.byteLength(readme,'utf8');assert(/Current release: \*\*v\d+\.\d+\*\*/.test(readme));assert(readme.includes('## Permanent North Star requirements'));assert(readme.includes('https://orange-cyberdefense.github.io/ocd-mindmaps/img/mindmap_ad_dark_classic_2025.03.excalidraw.svg'));assert(readme.includes('https://github.com/Orange-Cyberdefense/ocd-mindmaps/tree/main'));assert(readme.includes('<!-- OBOL-BUILD-NEXT:START -->'));assert(size<17000,`README remains too large: ${size}`);if(readme.includes('OBOL-PRODUCT-BUILD-NEXT:START'))assert(size<17000,`README product-hardening snapshot is too large: ${size}`);else assert(size<15000,`README remains too large: ${size}`);});
