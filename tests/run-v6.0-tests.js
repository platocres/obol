...
test('tiered workflow avoids full regressions on ordinary release commits',()=>{const workflow=fs.readFileSync(path.join(root,'.github','workflows','tests.yml'),'utf8');assert(/\n  smoke:\n/.test(workflow));assert(workflow.includes('node tools/release-smoke.js'));assert(workflow.includes("contains(github.event.head_commit.message, '[preflight]')"));assert(workflow.includes("contains(github.event.head_commit.message, '[release-final]')"));assert(workflow.includes('node tools/validate-historical-tests.js')||workflow.includes('node tools/run-historical-contracts.js'),'full regression gate must preserve historical contracts directly or through the current named runner');const preflightBlock=workflow.slice(workflow.indexOf('\n  preflight:'),workflow.indexOf('\n  test:'));assert(preflightBlock.includes("'[preflight]'"));assert(preflightBlock.includes("'[release-final]'"));});
...
test('current release repository contract passes',()=>{const r=cp.spawnSync(process.execPath,[path.join(root,'tools','validate-release-pr.js'),'--repo-only'],{cwd:root,encoding:'utf8'});assert.strictEqual(r.status,0,r.stderr||r.stdout);});
test('v6.0 release wiring remains present',()=>{const idx=fs.readFileSync(path.join(root,'index.html'),'utf8'),readme=fs.readFileSync(path.join(root,'README.md'),'utf8'),changelog=fs.readFileSync(path.join(root,'CHANGELOG.md'),'utf8');for(const x of ['methodology-v6.0.js','dashboard-v6.0.js','core-v6.0.js','intake-v6.0.js','app-v6.0.js','obol-v6.0.css'])assert(idx.includes(x),x);assert(/Current release: \*\*v\d+\.\d+(?:\.\d+)?\*\*/.test(readme));assert(readme.includes('BUILDING.md'));assert(changelog.includes('## v6.0'));for(const x of ['tools/release-smoke.js','tools/validate-historical-tests.js'])assert(fs.existsSync(path.join(root,x)),x);assert(fs.existsSync(path.join(root,'docs','v6.0.md')));});
test('historical README Build Next generator remains structurally valid after v6.0',()=>{const out=cp.execFileSync(process.execPath,[path.join(root,'tools','sync-readme-build-next.js'),'--print'],{encoding:'utf8'});assert(/\d+ canonical gaps/.test(out));assert(/\d+\/127 fully implemented \(\d+%\)/.test(out));assert(out.includes('**Current live queue:**'));assert(out.includes('**Highest-priority live items:**'));});
