...
test('tiered workflow avoids full regressions on ordinary release commits',()=>{const workflow=fs.readFileSync(path.join(root,'.github','workflows','tests.yml'),'utf8');assert(/\n  smoke:\n/.test(workflow));assert(workflow.includes('node tools/run-historical-contracts.js'),'main regression workflow must preserve the complete historical contract runner');assert(/push:\s*[\s\S]*branches:\s*[\s\S]*- main/.test(workflow),'the complete historical regression must run on main');assert(!/pull_request:/.test(workflow),'ordinary/PR commits must not trigger the complete historical regression here; PR gates live in full-regression-pr.yml');});
