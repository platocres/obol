...
const architectureTrack=q.tracks.find(track=>track.id==='architecture-runtime');assert(architectureTrack&&architectureTrack.complete===9&&architectureTrack.total===12,'runtime-retirement completion is represented in Architecture progress math');
...
assert.strictEqual(q.buildNext(1)[0].id,'notes-disposition-burn-down','Product Build Next returns to packetized Notes Integration after dashboard retirement');
const recommended=workPackages.recommend(q);assert(recommended&&recommended.id==='notes-impact-burn-down','next recommended package returns to Notes Impact and Themed Burn-down');
assert.deepStrictEqual(Array.from(recommended.liveItems).map(item=>item.id),['notes-disposition-burn-down','notes-packet-web-upload-inclusion','notes-packet-xss-session','notes-packet-credentials-auth','notes-packet-windows-privesc','notes-packet-linux-privesc','notes-packet-ad-pivoting'],'notes package exposes the umbrella and themed packets as live work');
...
console.log('v9.29 Notes Impact, stable Dashboard Current Owner, Browser Smoke, and completed Dashboard runtime retirement regression tests passed.');
