...
assert.strictEqual(itemMap.get('runtime-dashboard-layer-retirement').status,'queued','physical dashboard layer retirement remains active while dashboard data/core dependencies are still live');
...
const expectedRetiredDashboardPresentation=[...Array.from({length:9},(_,i)=>'assets/app-v5.'+(i+1)+'.js'),...['6.0','6.1','6.2','6.4','6.5'].map(v=>'assets/app-v'+v+'.js')];
assert.deepStrictEqual(Array.from(runtimeManifest.retiredStartupScripts||[]),expectedRetiredDashboardPresentation,'dashboard-only v5.1-v6.5 overlays are fixtures, not live startup');
assert.strictEqual(runtimeManifest.startupScripts.length,252,'operator compatibility startup shrinks by fourteen retired dashboard-only overlays');
for(const src of expectedRetiredDashboardPresentation){assert(runtimeManifest.scripts.includes(src),'retired dashboard overlay remains in historical ledger: '+src);assert(!runtimeManifest.startupScripts.includes(src),'retired dashboard overlay must not execute in live startup: '+src);}
...
console.log('v9.29 Notes Impact, stable Dashboard Current Owner, Browser Smoke, first presentation-layer retirement, and Runtime Retirement Governance regression tests passed.');
