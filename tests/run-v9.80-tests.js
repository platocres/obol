...
assert.strictEqual(wave.browserControlsIntegratedInto,'web-client-session-proof-chain');
...
assert.ok(commandRuns.some(run=>/content-security-policy/i.test(run)),'existing card should include CSP/header review command');
...
assert.ok((card.expected||[]).includes('CSP and browser-control behavior recorded'),'existing card should own browser-control proof');
assert.ok((card.produces||[]).includes('web.xss.browser_proof_reviewed'),'existing card should produce browser proof review fact');
assert.ok((card.produces||[]).includes('web.xss.csp_controls_reviewed'),'existing card should produce CSP review fact');
...
console.log('v9.80 XSS/client-session/CSP mechanics integrated into the existing session proof card without wrapper cards.');
