...
assert.deepStrictEqual(wave.primaryCardIds,['linux-privesc-boundary-sweep']);
assert.deepStrictEqual(wave.enrichedCardIds,['linux-privesc-boundary-sweep']);
assert.ok(wave.featureIds.includes('linux-privesc-signal-router'));
...
assert.ok(!globalThis.CARDS['linux-privesc-signal-router'],'signal router must remain a mechanic, not a duplicate primary card');
const card=globalThis.CARDS&&globalThis.CARDS['linux-privesc-boundary-sweep'];
assert.ok(card,'Linux privesc boundary sweep should remain the single primary card');
assert.strictEqual(card.cardKind,'primary');
assert.strictEqual(card.currentOwner,true);
assert.ok((card.featureIds||[]).includes('linux-privesc-signal-router'),'boundary sweep should carry the signal-router feature marker');
assert.ok(/sudo|SUID|capability|service|kernel|scheduled/i.test(card.operatorGoal+card.hypothesis+card.whyNow));
assert.ok((card.commands||[]).length>=10,'boundary sweep needs concrete operator commands');
for(const row of card.commands||[])assert.ok(row.tool&&row.run&&row.when&&row.evidence&&row.note,'Linux boundary command needs full action-spine fields');
...
for(const fact of ['linux.privesc_signal_reviewed','linux.sudo_rule_reviewed','linux.suid_candidate_reviewed','linux.capability_candidate_reviewed','linux.service_schedule_candidate_reviewed','linux.kernel_context_reviewed','cleanup.linux_privesc_artifacts_reviewed','report.linux_privesc_boundary_reviewed','linux.privesc_signal_router_handoff_reviewed'])assert.ok((card.produces||[]).includes(fact),'boundary sweep missing produced fact '+fact);
const cardText=JSON.stringify(card);
assert.ok(!/source-mining|source re-mining|release cleanup|patch panel|\bUNKNOWN\b|methodology gap|wrapper/i.test(cardText),'Linux boundary sweep must not leak implementation slop');
...
for(const note of notes.filter(note=>note&&wave.publicNoteIds.includes(note.id))){
 assert.deepStrictEqual(note.cardIds,['linux-privesc-boundary-sweep'],'v9.87 note binds to boundary sweep owner card');
 assert.deepStrictEqual(note.pathIds,['linux-privesc-boundary-sweep'],'v9.87 path note binds to boundary sweep owner card');
}
...
assert.strictEqual(analysis.cardId,'linux-privesc-boundary-sweep');
assert.strictEqual(analysis.featureId,'linux-privesc-signal-router');
...
assert.ok(intake&&Array.isArray(intake.activities)&&intake.activities.some(row=>row.cardId==='linux-privesc-boundary-sweep'),'intake should add Linux privesc boundary activity');
...
assert.deepStrictEqual(clusters.status.latestCompletedClusterOwnerCards,['linux-privesc-boundary-sweep']);
...
console.log('v9.87 mined Linux privilege escalation signal routing into the existing boundary sweep and advanced the cluster queue to Windows local privesc.');
