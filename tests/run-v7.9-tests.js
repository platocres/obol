...
test('v7.9 canonical advance remains limited to the two frozen low-access parents',()=>{assert.deepStrictEqual(P.sourceWave.canonicalAdvanced,['low_access.local-exploit','low_access.webdav']);for(const key of P.sourceWave.canonicalAdvanced){const s=section(key);assert(s,key);assert.strictEqual(s.status,'implemented',key);assert.strictEqual(s.advancedIn,'7.9',key);assert.strictEqual(s.sourceDepthAudit62.status,'modeled',key);}assert.strictEqual(P.sourceWave.filesAtomizedAfter,9);assert((F.atomizedFiles||[]).length>=P.sourceWave.filesAtomizedAfter);});
