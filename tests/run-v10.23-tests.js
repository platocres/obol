...
// Demoted from the current-release pin (v10.24 shipped): this stays a durable v10-line
// regression for the credential/auth functional + evidence repair without freezing an old literal.
...
assert(releaseSandbox.OBOL_RELEASE_IDENTITY.extensionPlan('tools').sources.includes('data/product-hardening/credential-auth-guidance-current.js'),'compact Tools route must keep loading the credential/auth schema-record repair');
...
assert(/\["tb-surface-(?:ad-smb|network|privesc)","tool-builders","queued"/.test(queue),'a later surface-repair family should remain queued after the credential/auth repair');
...
assert(changelog.includes('## v10.23'),'CHANGELOG must retain the v10.23 entry');
