...
// This guards the user-facing promise: the stale v10.09 queue label is not the
// next modeled-tool build anymore, and the v10.15 discovery tools are real
// builders with minimum commands, prefill, Evidence ingestion, and conservative
// proof boundaries.
...
assert.strictEqual(network.version,'v10.15','network discovery owner must identify v10.15');
...
for(const [idx,tool] of tools.entries()){
 ...
 assert.strictEqual(record.status,'implemented',tool+' must be promoted only by the v10.15 owner');
 assert.strictEqual(record.queueItem,ids[idx],tool+' must point at its v10.15 builder');
...
const context=Object.freeze({
 target:Object.freeze({value:'203.0.113.77',ip:'203.0.113.77',hostname:'lab-target.example'}),
 ...
 workspace:Object.freeze({wordlist:'/usr/share/wordlists/rockyou.txt',outputDir:'scans/v10.15',hashfile:'audit-hashes.txt',transferUrl:'http://198.51.100.77:8000/audit.bin'})
});
function compile(id,values){return api.compile(schema.get(id),values||{},context);}
function noFake(command){
 for(const bad of ['10.10.10.10','10.10.14.9','domain.local','Password123!','8846f7eaee8fb117ad06bdd830b7586c','hashes.txt'])assert(!command.includes(bad),'generated command leaked fake placeholder '+bad+': '+command);
 assert(!/\buser\b/.test(command),'generated command leaked fake username placeholder: '+command);
}
const commands={
 masscan:compile('tb-masscan'),
 rustscan:compile('tb-rustscan'),
 naabu:compile('tb-naabu'),
 fping:compile('tb-fping'),
 nbtscan:compile('tb-nbtscan')
};
...
assert.strictEqual(relCtx.OBOL_CURRENT_RELEASE.label,'v10.15','current release authority must advance to v10.15');
assert(relCtx.OBOL_CURRENT_RELEASE.productHardeningExtensions.includes('data/product-hardening/network-discovery-tool-builders-v10.15.js'),'current release must load the v10.15 network discovery owner');

const queue=read('docs/TOOL-BUILDER-BUILD-QUEUE.md');
assert(queue.includes('v10.15 network and host discovery slice'),'Tool Builder queue must name v10.15 as the completed/current discovery slice');
assert(!/v10\.09[^\n]*next modeled-tool build/i.test(queue),'queue must not describe v10.09 as the next modeled-tool build');
...
console.log('v10.15 network discovery Tool Builders, Evidence ingestion, queue handoff, audit compatibility, and release validation passed.');
