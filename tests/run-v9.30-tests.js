...
assert.strictEqual(notes.schemaVersion,'1.4.0');
...
assert.strictEqual(notes.ledger.reviewedCount,65);
assert.strictEqual(notes.ledger.dispositionCounts.modeled,48);
assert.strictEqual(notes.ledger.dispositionCounts['private-reference-only'],13);
...
assert.strictEqual(notes.ledger.dispositionCounts['pending-review'],491);
...
assert.strictEqual(impact.review.reviewed,65);
assert.strictEqual(impact.review.pending,491);
assert.strictEqual(impact.outputCounts.fieldNotes,29);
...
assert.strictEqual(impact.outputCounts.explicitGuidanceOnlyDecisions,4);
assert.strictEqual(impact.latestWave.id,'v9.30-web-upload-inclusion-1');
assert.strictEqual(impact.latestWave.reviewed,10);
assert.strictEqual(impact.latestWave.modeled,5);
assert.strictEqual(impact.latestWave.privateOnly,1);
assert.strictEqual(impact.latestWave.productChanges.length,1);
assert.strictEqual(impact.latestWave.productChanges[0].type,'tool-builder-change');
assert.deepStrictEqual(Array.from(impact.latestWave.productChanges[0].proofRefs),['assets/tool-builder-current.js','tests/run-v9.30-tests.js']);
for(const id of ['note-path-resolution-baseline','note-lfi-stack-path-hypotheses','note-path-transport-normalization','note-file-valued-parameter-triage','note-upload-overwrite-boundary']){
 assert(impact.outputs.some(output=>output.id===id),'v9.30 public-safe note output exists: '+id);
 assert(impact.latestWave.outputs.includes(id),'v9.30 latest-wave projection includes '+id);
}

const transportDecision=impact.sourceDecisions.find(row=>row.noteId==='offsec-pen-200-4940931777995183');
assert(transportDecision&&transportDecision.productChanges.length===1,'path transport note declares a code-level product change');
...
assert(routeOwner.includes('reviewSchemaAtLeast(1,3)'),'dashboard route accepts compatible current notes-review schemas instead of freezing v1.3.0');
...
for(const forbidden of ['OS{','PEN-200.enex','HTB - Penetration Tester.enex','review_text'])assert(!reviewSource.includes(forbidden),'public review owner excludes raw/private source material: '+forbidden);
