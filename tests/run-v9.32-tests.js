...
assert.strictEqual(release.version,'9.32.0');assert.strictEqual(release.label,'v9.32');
assert.strictEqual(notes.schemaVersion,'1.6.0');assert.deepStrictEqual(Array.from(notes.validate()),[]);
assert.strictEqual(notes.ledger.reviewedCount,90);assert.strictEqual(notes.ledger.dispositionCounts.modeled,63);assert.strictEqual(notes.ledger.dispositionCounts['private-reference-only'],23);assert.strictEqual(notes.ledger.dispositionCounts.superseded,4);assert.strictEqual(notes.ledger.dispositionCounts['pending-review'],466);
...
assert.strictEqual(impact.review.reviewed,90);assert.strictEqual(impact.review.pending,466);assert.strictEqual(impact.outputCounts.fieldNotes,35);assert.strictEqual(impact.latestWave.id,'v9.32-xss-session');assert.strictEqual(impact.latestWave.reviewed,14);assert.strictEqual(impact.latestWave.modeled,10);assert.strictEqual(impact.latestWave.privateOnly,4);assert.strictEqual(impact.latestWave.productChanges.length,0);assert(impact.latestWave.themes.includes('XSS / session hardening'));
...
console.log('v9.32 XSS/session notes packet, proof boundaries, public-safe guidance, queue projection, and release contract tests passed.');
