...
const packetItem=q.items.find(item=>item.id==='notes-packet-xss-session');assert(packetItem);assert.strictEqual(packetItem.status,'complete');assert(q.buildNext(q.items.length).some(row=>row.id==='notes-disposition-burn-down'),'notes disposition burn-down remains live queue work');
