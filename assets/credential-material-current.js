...
function decorateBuilder(section){
 ...
 host.innerHTML='<div class="card-body"><div class="section-head30"><div><span class="eyebrow30">Credential Material</span><h3>Reuse candidate material</h3><p class="hint">Save a password, hash, ticket, key, certificate, cookie, or token once, then hand it to compatible builders without retyping it. Material remains candidate until independent reviewed Evidence proves access.</p></div></div>'+
  ...
  '<div class="modal-actions"><button type="button" class="btn" data-credential-apply>Use selected in this builder</button></div>'+
 ...
 const select=host.querySelector('[data-credential-select]'),value=host.querySelector('[data-credential-value]'),kind=host.querySelector('[data-credential-kind]'),status=host.querySelector('[data-credential-status]');
 function refresh(){const latest=api.list(s),sel=api.selected(s);select.innerHTML=optionRows(latest,sel&&sel.id);status.textContent=(sel?'Selected: '+visibleLabel(sel)+'. ':'')+'Generated commands and saved material are workflow activity, not proof. Validation requires independent reviewed Evidence and an explicit access fact.';}
 select.addEventListener('change',()=>{api.select(s,select.value);persist();refresh();});
 host.querySelector('[data-credential-apply]').addEventListener('click',()=>{const row=api.selected(s);if(!row){status.textContent='Choose saved material first.';return;}const values=api.prefillForBuilder(row,builder);if(!Object.keys(values).length){status.textContent='The selected material has no compatible fields on this builder.';return;}applyValues(form,builder,values);persist();status.textContent='Selected material handed to compatible fields. It remains '+row.status+' and is not proof of access.';});
