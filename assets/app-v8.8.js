...
function ensureProductAssets88(){return ensureRelease88().then(()=>{if(window.OBOL_CURRENT_WORKFLOW)return window.OBOL_CURRENT_WORKFLOW;if(productAssetsLoading)return productAssetsLoading;addStyle88('assets/product-hardening-dashboard.css');productAssetsLoading=addScript88('data/product-hardening/product-hardening-queue.js').then(()=>addScript88('data/product-hardening/work-packages.js')).then(()=>addScript88('assets/product-hardening-dashboard.js')).then(()=>addScript88(WORKFLOW_SOURCE)).then(()=>window.OBOL_CURRENT_WORKFLOW);return productAssetsLoading;});}
function decorate88(){if(!active88())return;Promise.all([ensureRelease88(),ensureProductAssets88()]).then(()=>{setVisibleVersion88();const workflow=window.OBOL_CURRENT_WORKFLOW;if(workflow&&typeof workflow.decorateRoute==='function')workflow.decorateRoute();}).catch(()=>{});}
...
ensureAccessibility88().catch(()=>{});ensureRelease88().catch(()=>{});ensureProductAssets88().catch(()=>{});for(const t of [50,350,760,1300,2200,3600,5200])setTimeout(decorate88,t);
})();
