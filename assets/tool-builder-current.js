...
function html(builder,context,values){
 ...
 return '<section class="card tool-builder-current" data-tool-builder="'+esc(builder.id)+'" data-tool="'+esc(builder.tool)+'"><div class="card-body">'+
  '<div class="tool-builder-head"><div><span class="eyebrow30">Tool Builder</span><h3>'+esc(builder.title)+'</h3><p class="hint">'+esc(builder.summary)+'</p></div><span class="badge tool-exec-badge" title="Recommended run environment">'+esc(builder.executionContext||'any')+'</span></div>'+creds+renderOperatorGuide(builder,resolved)+
