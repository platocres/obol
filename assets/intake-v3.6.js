...
function rubeusIntent36(command){const c=norm36(command);if(!/\brubeus(?:\.exe)?\b/.test(c))return'';if(/\basreproast\b/.test(c))return'asrep-roast';if(/\bkerberoast\b/.test(c))return'kerberoast';if(/\basktgt\b/.test(c)||/\bptt\b/.test(c))return'kerberos-tickets';if(/\bs4u\b/.test(c))return'delegation-abuse';return'';}
