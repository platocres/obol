...
(async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(()=>({frames:window.__OBOL_VISIBLE_BOOT_FRAMES__.slice(),boot:document.documentElement.dataset.obolBoot,paint:document.documentElement.dataset.obolCurrentPaint,first:window.__OBOL_CURRENT_FIRST_VISIBLE_ROUTE__,title:document.title,tagline:(document.querySelector('.tagline')||{}).textContent||'',current:!!document.querySelector('#view [data-current-home-owner="workflow-current"]'),text:(document.getElementById('view').innerText||'').slice(0,200)}));
  ...
  await context.close();
  console.log('Single-paint boot valid: delaying workflow-current.js by 1200ms produced no visible historical Home frame before the explicit current route owner claimed first paint.');
