...
assert.strictEqual(notes.schemaVersion,'1.7.0');
...
assert.strictEqual(notes.ledger.reviewedCount,112);
assert.strictEqual(notes.ledger.dispositionCounts.modeled,82);
assert.strictEqual(notes.ledger.dispositionCounts['private-reference-only'],25);
...
assert.strictEqual(notes.ledger.dispositionCounts['pending-review'],444);
assert.strictEqual(notes.reviewedDispositions.length,112);
...
const milestones=[
 ['v9.25',4,4,0,0,552],['v9.26-wave-1',15,11,4,0,541],['v9.27-wave-2',41,32,9,0,515],['v9.28-wave-3',55,43,12,0,501],['v9.30-web-upload-inclusion-1',65,48,13,4,491],['v9.30-web-upload-inclusion-2',76,53,19,4,480],['v9.32-xss-session',90,63,23,4,466],['v9.33-credentials-auth',112,82,25,5,444]
];
...
assert.strictEqual(field.entries.length,43,'forty-three normalized public field notes are exposed');
...
console.log('Notes integration valid: explicit 112/556 disposition ledger, 43 public-safe notes, completed credentials/auth packet, preserved milestones, and raw-source boundary are intact.');