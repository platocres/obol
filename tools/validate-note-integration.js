...
assert.strictEqual(notes.ledger.reviewedCount,65,'current ledger records sixty-five terminal note dispositions');
assert.strictEqual(notes.ledger.dispositionCounts.modeled,48,'forty-eight reviewed notes are modeled');
assert.strictEqual(notes.ledger.dispositionCounts['private-reference-only'],13,'thirteen reviewed notes remain private-reference-only');
...
assert.strictEqual(notes.ledger.dispositionCounts['pending-review'],491,'unreviewed notes remain explicitly pending');
assert.strictEqual(notes.reviewedDispositions.length,65,'explicit disposition rows back the reviewed count');
...
assert.strictEqual(field.entries.length,29,'twenty-nine normalized public field notes are exposed');
...
for(const id of ['note-path-resolution-baseline','note-lfi-stack-path-hypotheses','note-path-transport-normalization','note-file-valued-parameter-triage','note-upload-overwrite-boundary'])assert(field.entries.some(entry=>entry.id===id),'v9.30 derived field note exists: '+id);
...
console.log('Notes integration valid: explicit 65/556 disposition ledger, 29 public-safe derived notes, preserved milestones, and raw-source boundary are intact.');
