...
for(const rel of ['data/tool-builder-schema.js','data/tool-builder-inventory.js','assets/tool-builder-current.js','data/product-hardening/product-hardening-queue.js'])vm.runInContext(read(rel),sandbox,{filename:rel});
...
assert(schema&&inventory&&renderer&&queue,'Tool Builder platform owners must initialize');
...
for(const required of ['data/tool-builder-schema.js','data/tool-builder-inventory.js','assets/tool-builder-current.js'])assert(bridge.includes(required),'current browser bridge does not load Tool Builder owner: '+required);

console.log(`Tool Builder Platform valid: ${observed.size} runnable tool identities have explicit dispositions; schema, renderer, command compiler, human-run boundary, queue references, and representative inventory coverage are locked.`);
