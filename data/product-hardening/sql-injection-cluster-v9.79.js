'use strict';
(function(root){
const WAVE='v9.79-sql-injection-discovery-extraction-cluster';
const ACTIVE_QUEUE_ID='source-note-cluster-sql-injection-discovery-and-extraction';
const ACTIVE_CLUSTER_ID='sql-injection-discovery-and-extraction';
const CARD_ID='sqlmap-request-proof';
const FALLBACK_CARD_ID='web-authz-boundaries';
const ANALYZER_ID='sql-injection-evidence-analyzer-v979';
const SOURCE_ROUTE='platocres/obol-source-notes@agent/review-packets:data/review-packets/manifest.json';
const SOURCE_PACKETS=Object.freeze(['data/review-packets/htb-penetration-tester-05.json','data/review-packets/htb-penetration-tester-06.json']);
const REVIEW_TEXT_CHARS=265833;
const TOTAL_NOTES=38;
const PUBLIC_NOTE_IDS=Object.freeze([
 'note-sqli-hypothesis-triage-v979',
 'note-sqli-column-visibility-proof-v979',
 'note-sqli-database-enumeration-ladder-v979',
 'note-sqli-sqlmap-request-handoff-v979',
 'note-sqli-blind-error-time-boundaries-v979',
 'note-sqli-file-and-shell-risk-boundary-v979',
 'note-sqli-report-remediation-v979'
]);
...
const PUBLIC_NOTES=fl([
 ...
 note('note-sqli-column-visibility-proof-v979','Separate column count from visible output placement','For UNION work, first learn how many columns the original query returns, then learn which returned positions are actually rendered. ORDER BY and NULL or number probes answer column-count questions; reflected marker values answer where extracted data can be placed.','tool-guidance',['sql-injection','union','columns','evidence'],['curl','Burp Repeater']),
 note('note-sqli-database-enumeration-ladder-v979','Use a database enumeration ladder','After a printable SQLi is proven, enumerate deliberately: DBMS/version, current user and database, database names, table names, column names, then only the smallest rows needed to prove impact. Keep each step tied to the prior proof so extraction does not outrun evidence.','lesson',['sql-injection','enumeration','information-schema','database'],['sqlmap','curl']),
 note('note-sqli-sqlmap-request-handoff-v979','Hand sqlmap the request you actually proved','Use sqlmap after manual evidence identifies a likely injection point or when a captured request preserves cookies, headers, body fields, CSRF material, and method semantics better than a URL-only command. Mark the parameter with an asterisk when needed and keep batch output as triage until the finding is reviewed.','tool-guidance',['sqlmap','request-file','handoff','proof-boundary'],['sqlmap','Burp Repeater']),
 note('note-sqli-blind-error-time-boundaries-v979','Classify boolean, error, time, and UNION evidence separately','Boolean, error-based, time-based, stacked-query, and UNION detections prove different things. Record the technique class, the control request, the payload/request pair, and whether the response shows data, error detail, timing drift, or only tool confidence.','evidence',['sql-injection','blind','error-based','time-based','union']),
 note('note-sqli-file-and-shell-risk-boundary-v979','Gate file read, file write, and shell claims behind privileges','Database file operations and OS shell attempts are a separate, higher-risk branch. Check DBMS type, effective database user, FILE-like privileges, configured read/write directory restrictions, webroot uncertainty, and write-location proof before claiming file access or command execution.','cleanup',['sql-injection','file-read','file-write','os-shell','risk'],['sqlmap','curl']),
...
function analyze(text){
 ...
 if(/\b(sqlmap|Parameter:\s*\w+|appears to be injectable|Payload:|Type:\s*(boolean|error|time|UNION|stacked)|--risk|--level|--tamper|--batch|-r\s+\S+)\b/i.test(raw))add('sqlmap-handoff','sqlmap request handoff or finding output observed','web.sqli.sqlmap_output_observed');
 if(/\b(boolean-based blind|time-based blind|SLEEP\s*\(|BENCHMARK\s*\(|WAITFOR\s+DELAY|true condition|false condition|response time|delayed)\b/i.test(raw))add('blind-or-time','Boolean or time-based SQLi evidence observed','web.sqli.blind_or_time_evidence_observed');
 if(/\b(load_file|into\s+outfile|secure_file_priv|--file-read|--file-write|--file-dest|--os-shell|FILE privilege|webroot|written successfully|uid=|www-data)\b/i.test(raw))add('file-or-shell','Database file operation or OS-shell branch observed','web.sqli.file_or_shell_branch_observed');
 if(/\b(dump|dumped|entries|available databases|Database:\s+\w+|Table:\s+\w+|columns|rows retrieved|password hash|credential)\b/i.test(raw))add('data-extraction','Data extraction or dump output observed','web.sqli.data_extraction_observed');
 const outcomeFacts=fl(uniq(matches.map(m=>m.fact))),warnings=[];
 if(outcomeFacts.includes('web.sqli.error_or_syntax_observed'))warnings.push('Syntax-sensitive output is a SQLi lead; pair it with a control request before claiming exploitability.');
 if(outcomeFacts.includes('web.sqli.union_column_probe_observed'))warnings.push('Column-count success is not data exposure until a visible output position returns controlled or database data.');
 if(outcomeFacts.includes('web.sqli.sqlmap_output_observed'))warnings.push('sqlmap confidence needs reviewed request/response proof before becoming report-ready evidence.');
 if(outcomeFacts.includes('web.sqli.file_or_shell_branch_observed'))warnings.push('File and shell branches require explicit privilege, path, and execution proof and should stay scoped and reversible.');
