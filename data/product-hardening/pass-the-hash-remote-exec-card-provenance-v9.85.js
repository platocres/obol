...
function patchWave(){
 ...
 root.OBOL_PASS_THE_HASH_REMOTE_EXEC_CLUSTER_V985=frozen(Object.assign({},prev,{primaryCardId:PTH,enrichedCardIds:freeze([PTH]),ownerCardIds:freeze([PTH]),foldedAliasIds:freeze(FOLDED),cardTaxonomy:frozen({primary:PTH,origin:'legacy',enrichedBy:freeze(['v9.85']),foldedAliases:freeze(FOLDED)}),remoteIntegrated:true,tokenIntegrated:true,foldedAliases:true,failures:freeze((prev.failures||[]).filter(Boolean))}));
 return true;
}
...
function run(){
 ...
 root.OBOL_PTH_REMOTE_EXEC_CARD_PROVENANCE_V985=frozen({wave:'v9.85-pass-the-hash-folded-alias-cleanup',status:pthClean&&aliasesRemoved&&notesNormalized&&wavePatched&&analyzerPatched?'live-integrated':'partial',cardIds:freeze([PTH]),foldedAliasIds:freeze(FOLDED),cardTaxonomy:frozen({primary:PTH,primaryKind:'primary',primaryOrigin:'legacy',currentOwner:true,enrichedBy:freeze(['v9.85']),foldedAliases:freeze(FOLDED)}),pthClean,aliasesRemoved,notesNormalized,analyzerPatched,wavePatched,clustersPatched});
