...
function publishPth(){
 ...
 if(!placed){let lane=lanes().find(entry=>entry&&(entry.id==='credentials'||entry.lane==='credentials'));if(!lane){lane={id:'credentials',lane:'credentials',title:'Credentials',cards:[]};if(Array.isArray(root.OBOL_LANES))root.OBOL_LANES.push(lane);else root.OBOL_LANES=[lane];}if(!Array.isArray(lane.cards))lane.cards=[];lane.cards.push(card);}
 return !dirty(card);
}
function removeFoldedAliases(){
 if(root.CARDS&&typeof root.CARDS==='object')for(const id of FOLDED)delete root.CARDS[id];
 for(const lane of lanes())if(Array.isArray(lane.cards))lane.cards=lane.cards.filter(card=>!(card&&FOLDED.includes(card.id)));
 return FOLDED.every(id=>!(root.CARDS&&root.CARDS[id]));
}
function normalizeNotes(){
 ...
 holder.publicFieldNotes=freeze(holder.publicFieldNotes.map(note=>{
  if(!note||!PUBLIC_NOTES.includes(note.id))return note;
