...
const BLOCKED_PLACEHOLDERS=Object.freeze([
 '10.10.10.10','10.10.14.9','domain.local','user','Password123!',
 '8846f7eaee8fb117ad06bdd830b7586c',':8846f7eaee8fb117ad06bdd830b7586c','hashes.txt'
]);
...
function words(value){return text(value).split(/\s+/).map(v=>v.replace(/["']/g,''));}
function basename(value){return text(value).replace(/["']/g,'').replace(/^.*[\\/]/,'');}
function hasBlockedPlaceholder(value){
 ...
 return BLOCKED_PLACEHOLDERS.some(p=>{
  ...
  if(p==='hashes.txt')return words(raw).some(t=>basename(t)==='hashes.txt');
...
function contextFromState(state){
 const raw=state||readRawState();
 const params=paramsFromState(raw);
 ...
 return Object.freeze({
  ...
  context:Object.freeze({domain,username,port:first(params,['port','rport','RPORT']),lhost,lport,baseDn:domainToBaseDn(domain)}),
...
function safeDefaults(builder,context,values){
 ...
 out=Object.assign(out,scrub(values||{}));
 return scrub(out);
}
function minimumFixtureValues(builder,context){
 ...
 fill('url',builder&&builder.id==='tb-file-transfer-helper'?context.workspace&&context.workspace.transferUrl||'':context.target&&context.target.value||'');
 ...
 fill('wordlist',context.workspace&&context.workspace.wordlist||'/usr/share/wordlists/rockyou.txt');
 for(const field of arr(builder&&builder.fields)){
  ...
  else if(field.type==='path')values[field.id]=field.id.toLowerCase().includes('file')?'audit-supplied-file.txt':'audit-supplied-path';
  else if(field.type==='number')values[field.id]='1';
  else if(field.type==='select'&&arr(field.options).length)values[field.id]=field.default||field.options[0].value;
...
function inventoryRecords(){const inv=root.OBOL_TOOL_BUILDER_INVENTORY;if(!inv||typeof inv.all!=='function')return[];return inv.all();}
...
function validateImplementedBuilders(){
 ...
 for(const record of implementedRecords()){
  ...
  try{
   const values=minimumFixtureValues(builder,context);
   const command=compileMinimum(builder,context,values);
   if(hasBlockedPlaceholder(command))failures.push(builder.id+' generated blocked placeholder in minimum command: '+command);
   if(!command||command.split(/\s+/).length<1)failures.push(builder.id+' generated empty minimum command');
   const targetReferenced=['target','rhost','rhosts','url'].some(id=>commandUsesField(builder,id,values));
   if(targetReferenced&&!/(203\.0\.113\.77|http:\/\/203\.0\.113\.77|198\.51\.100\.77)/.test(command))failures.push(builder.id+' did not prefill target/url from supplied or parsed context');
...
function activeBatches(){
 const modeled=modeledRecords().map(record=>record.tool).sort();
...
function auditSnapshot(){const context=contextFromState();return Object.freeze({version:VERSION,implementedCount:implementedRecords().length,modeledCount:modeledRecords().length,failures:validateImplementedBuilders(),activeBatches:activeBatches(),context});}
...
function repairToolsRoute(){
 ...
 const api=root.OBOL_TOOLS_LIBRARY_CURRENT,builderApi=root.OBOL_TOOL_BUILDER;
 if(!api||!builderApi||typeof api.builderForTool!=='function')return false;
...
function patchQueue(){
 ...
 target.priority=18.861;
 target.label='Post-mining modeled tool builder implementation backlog';
 target.detail='v10.08 completed the implemented-builder audit only. '+modeled.length+' inventory records still remain modeled and must be implemented or explicitly superseded/rejected with the full Tool Builder contract before this backlog can close.';
 ...
 delete target.proof;
 const track=Array.isArray(q.tracks)?q.tracks.find(entry=>entry&&entry.id==='tool-builders'):null;
 if(track){track.total=Math.max(Number(track.total)||0,Number(track.complete||0)+1);track.complete=Math.min(Number(track.complete)||0,Math.max(0,Number(track.total)||0-1));}
 return true;
}
