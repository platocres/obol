...
function hostFromUrl(value){const raw=text(value);const m=raw.match(/^https?:\/\/([^\/:?#]+)(?::\d+)?(?:[\/?#]|$)/i);return m?m[1]:'';}
...
function contextFromState(state){
 ...
 return Object.freeze({
  ...
  workspace:Object.freeze({wordlist:first(params,['wordlist','wordlistPath'])||'/usr/share/wordlists/rockyou.txt',outputDir:first(params,['outputDir','lootDir','scansDir'])||'',hashfile:first(params,['hashfile','hashFile','hashes','hashPath']),transferUrl:first(params,['transferUrl','url','downloadUrl'])})
 });
}
...
function derivedFieldValue(field,context){
 ...
 if(id==='url')return context.workspace&&context.workspace.transferUrl||context.target&&context.target.value||'';
...
function safeDefaults(builder,context,values){
 ...
 if(registry&&typeof registry.defaultsFor==='function'){
  try{out=Object.assign(out,registry.defaultsFor(builder.id,{},context));}catch(_err){}
 }
 for(const field of arr(builder&&builder.fields)){
  const derived=derivedFieldValue(field,context);if(derived&&out[field.id]===undefined)out[field.id]=derived;
 }
