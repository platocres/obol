...
const V1002_PROOF='data/product-hardening/visual-density-regression-v10.02.js';
...
const VISUAL_CLOSEOUT=Object.freeze({id:VISUAL_ITEM,track:'testing-qa',status:'complete',priority:18.94,completedBy:'v10.02',proof:V1002_PROOF,label:'Post-mining visual density regression pass',detail:'Complete in v10.02: the browser smoke now captures Path, Card, and Tools visual-density screenshots and fails on reintroduced matching-card dumps, wrapper panels, hidden primary actions, missing Evidence loops, missing tool accessories, or obvious responsive overflow. The proof keeps the cleanup honest without changing the operator surfaces themselves.',acceptance:'Browser proof covers Path, Card, and Tools after the post-notes cleanup sequence. It fails on reintroduced matching-card dumps, wrapper panels, hidden primary actions, missing evidence loops, missing tool accessories, and narrow-layout overflow while preserving action, evidence, accessories, and decision guidance.'});
...
const V1002_AUDIT=Object.freeze([
 {id:'path-density-proof',surface:'Path route',proof:'Path screenshots cover desktop and narrow viewports; the best next move stays dominant and Evidence needs remain compact.',status:'complete'},
 {id:'card-density-proof',surface:'Card route',proof:'Representative card screenshots prove why-now, primary action, primary command, Evidence loop, and secondary details stay ordered without wrapper panels.',status:'complete'},
 {id:'tools-density-proof',surface:'Tools route',proof:'Tools home and selected tool screenshots prove grouped direct launch, builder-first detail, accessories, modes, and collapsed legacy examples.',status:'complete'},
 {id:'dump-regression-guard',surface:'Browser smoke',proof:'The visual-density smoke fails if selected Tools routes render expanded matching-card dumps or cleanup wrapper panels return.',status:'complete'}
]);
...
function patchToolsItem(tools){if(!tools)return false;for(const [key,value] of Object.entries(TOOLS_CLOSEOUT))setValue(tools,key,value);setValue(tools,'libraryShape','grouped-builder-launcher');setValue(tools,'preservesDirectToolSelection',true);setValue(tools,'accessoriesFirstClass',true);setValue(tools,'legacyMatchingCards','collapsed-drilldown-only');setValue(tools,'nextConcrete',TOOL_BACKLOG_ITEM);setValue(tools,'followUpItem',TOOL_BACKLOG_ITEM);return true;}
function patchVisualItem(visual){if(!visual)return false;for(const [key,value] of Object.entries(VISUAL_CLOSEOUT))setValue(visual,key,value);setValue(visual,'screenshots',['visual-density-path-desktop.png','visual-density-path-narrow.png','visual-density-card-web-authz-desktop.png','visual-density-card-pivot-desktop.png','visual-density-tools-home-desktop.png','visual-density-tools-ffuf-desktop.png','visual-density-tools-hashcat-desktop.png','visual-density-tools-ligolo-desktop.png']);setValue(visual,'followUpItem',TOOL_BACKLOG_ITEM);return true;}
...
function patchQueue(){
 ...
 const visualClosed=patchVisualItem(upsert(q.items,VISUAL_CLOSEOUT));
 ...
 const qaTrack=arr(q.tracks).find(t=>t&&t.id==='testing-qa');
 if(qaTrack&&Number(qaTrack.total)>=12)setValue(qaTrack,'complete',Math.max(Number(qaTrack.complete)||0,9));
 return cardClosed&&!!retire&&toolsClosed&&visualClosed&&!!backlog;
}
function patchPackages(){
 ...
 setValue(pkg,'guidance','Path cleanup, Card progressive disclosure, Card wrapper/decorator retirement, Tools builder-library cleanup, and visual density regression proof have landed. Next, execute the modeled tool builder implementation backlog so high-value modeled tools graduate into implemented builders without weakening direct tool selection, shared accessories, or Evidence paste-back guidance.');
 return true;
}
...
root.OBOL_CARD_WRAPPER_RETIREMENT_QUEUE_V998=Object.freeze({version:QUEUE_VERSION,status:applied?'complete':'not-applied',cardItem:CARD_ITEM,retirementItem:RETIRE_ITEM,toolsItem:TOOLS_ITEM,visualItem:VISUAL_ITEM,toolBacklogItem:TOOL_BACKLOG_ITEM,wrapperLedger:WRAPPER_LEDGER,applied,packagesPatched});
...
root.OBOL_CARD_WRAPPER_DECORATOR_RETIREMENT_V100=Object.freeze({version:'v10.0',status:retirementItem&&retirementItem.status==='complete'?'complete':'not-applied',item:RETIRE_ITEM,completedBy:'v10.0',proof:V100_PROOF,audit:V100_AUDIT,wrapperLedger:WRAPPER_LEDGER,toolsNext:!!toolsItem,packagesPatched,requestBudgetNeutral:true});
root.OBOL_TOOLS_BUILDER_LIBRARY_CLEANUP_V1001=Object.freeze({version:'v10.01',status:toolsItem&&toolsItem.status==='complete'?'complete':'not-applied',item:TOOLS_ITEM,completedBy:'v10.01',proof:V1001_PROOF,audit:V1001_AUDIT,nextItem:VISUAL_ITEM,visualDensityNext:!!visualItem,toolBacklogItem:TOOL_BACKLOG_ITEM,toolBacklogQueued:!!toolBacklogItem&&toolBacklogItem.status==='queued',directToolSelection:true,implementedBuildersFirst:true,accessoriesFirstClass:true,legacyMatchingCardsCollapsed:true,requestBudgetNeutral:true,packagesPatched});
root.OBOL_VISUAL_DENSITY_REGRESSION_V1002=Object.freeze({version:'v10.02',status:visualItem&&visualItem.status==='complete'?'complete':'not-applied',item:VISUAL_ITEM,completedBy:'v10.02',proof:V1002_PROOF,audit:V1002_AUDIT,browserProof:'tests/playwright-visual-density.js',nextItem:TOOL_BACKLOG_ITEM,toolBacklogNext:buildNext.some(item=>item&&item.id===TOOL_BACKLOG_ITEM),screenshots:visualItem&&visualItem.screenshots||[],requestBudgetNeutral:true,packagesPatched});
root.OBOL_TOOL_BUILDER_IMPLEMENTATION_BACKLOG_V1001=Object.freeze({version:'v10.02',status:toolBacklogItem&&toolBacklogItem.status==='queued'?'queued':'not-applied',item:TOOL_BACKLOG_ITEM,after:VISUAL_ITEM,sourceInventory:'data/tool-builder-inventory.js',sharedRenderer:'assets/tool-builder-current.js',directLauncher:'assets/tools-library-current.js',modeledToolsRemain:true,sharesEvidenceContract:true,sharesAccessoryContract:true,sharesCommandRenderer:true,visualDependency:!!visualItem,visualDependencyComplete:!!visualItem&&visualItem.status==='complete'});
