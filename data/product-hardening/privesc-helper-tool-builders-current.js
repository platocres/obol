...
function tb(id,tool,title,summary,fields,tokens,expectation,proofBoundary,secrets){return Object.assign({id,tool,title,summary,executionContext:'kali',credentialModes:[],fields,command:{executable:tool,tokens}},common(expectation,proofBoundary,secrets||[]));}
function builderDefs(){return [
 tb('tb-pspy','pspy','pspy process watcher builder','Build a bounded pspy process-watch command for Linux privilege-escalation observation. The default requires an explicit binary path on the target and optional duration/output controls are additive.',[
  ...
  f('duration','Timeout seconds','number',{placeholder:'120',visibleWhen:{field:'mode',equals:'run'}}),
  f('outputFile','Output file','path',{placeholder:'/tmp/pspy.out',visibleWhen:{field:'mode',equals:'run'}}),
  ...
  {kind:'field',field:'duration',prefix:'timeout ',raw:false,when:{field:'mode',equals:'run'}},
  ...
  {kind:'literal',value:'chmod +x',when:{field:'mode',equals:'chmod'}},{kind:'field',field:'binary',when:{field:'mode',equals:'chmod'}},
  {kind:'literal',value:'rm -f',when:{field:'mode',equals:'cleanup'}},{kind:'field',field:'binary',when:{field:'mode',equals:'cleanup'}},{kind:'field',field:'outputFile',when:{field:'cleanupOutput',truthy:true}}
 ...
 tb('tb-accesschk','accesschk','accesschk permission review builder','Build focused Windows Sysinternals accesschk commands for services, files, directories, registry keys, users/groups, and cleanup-aware permission review.',[
  f('target','Object, path, service, or principal','text',{required:true,placeholder:'C:\\Program Files\\Example\\service.exe'}),
  f('mode','Review mode','select',{default:'file',options:[opt('file','File or directory ACL'),opt('service','Service permissions'),opt('registry','Registry key permissions'),opt('user','User/group rights'),opt('cleanup','Cleanup copied accesschk binary')]}),
  f('binary','accesschk binary path','path',{default:'accesschk.exe'}),
  ...
  {kind:'field',field:'binary',when:{field:'mode',truthy:true}},
  {kind:'toggle',field:'acceptEula',flag:'-accepteula'},
  {kind:'toggle',field:'quiet',flag:'-nobanner'},
  {kind:'choice',field:'mode',choices:[{value:'file',arg:''},{value:'service',arg:'-c'},{value:'registry',arg:'-k'},{value:'user',arg:'-a'},{value:'cleanup',arg:'/c del /f /q'}]},
 ...
 tb('tb-searchsploit','searchsploit','searchsploit exploit reference builder','Build searchsploit lookup, exact-match, path-print, mirror, and update commands while treating exploit references as candidate research leads only.',[
  f('query','Search query or exploit path','text',{required:true,placeholder:'OpenSSH 7.2p2'}),
  f('mode','Mode','select',{default:'search',options:[opt('search','Search exploit titles'),opt('exact','Exact title search'),opt('path','Print exploit path'),opt('mirror','Mirror exploit locally'),opt('update','Update exploit-db copy')]}),
  ...
  {kind:'choice',field:'mode',choices:[{value:'search',arg:''},{value:'exact',arg:'--exact'},{value:'path',arg:'-p'},{value:'mirror',arg:'-m'},{value:'update',arg:'-u'}]},
  ...
  {kind:'field',field:'query',when:{field:'mode',truthy:true}}
...
function patchInventory(){
 ...
 const updates={
  pspy:inventoryUpdate('pspy','tb-pspy','pspy is implemented as a bounded Linux process-observation helper with run/chmod/cleanup controls and Evidence-gated privesc-lead boundaries.'),
  accesschk:inventoryUpdate('accesschk','tb-accesschk','accesschk is implemented as a Windows permission-review helper with file, service, registry, user/right, and cleanup-aware Evidence boundaries.'),
  searchsploit:inventoryUpdate('searchsploit','tb-searchsploit','searchsploit is implemented as an exploit-reference lookup helper with path/mirror/update controls and strict candidate-reference proof boundaries.'),
  linpeas:inventoryUpdate('linpeas','tb-linpeas','linpeas remains implemented through the canonical helper builder owner; v10.19 preserves its disposition while clearing the visible privilege-helper modeled slice.'),
  winpeas:inventoryUpdate('winpeas','tb-winpeas','winPEAS remains implemented through the canonical helper builder owner; v10.19 preserves its disposition while clearing the visible privilege-helper modeled slice.')
 };
...
function registerBuilders(){
 ...
 if(root.__OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT_REGISTERED__){patchInventory();return true;}
 ...
 patchInventory();
 root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT=Object.freeze({version:VERSION,builders:Object.freeze(builders),tools:TOOLS,builderIds:IDS,preservedTools:PRESERVED_TOOLS,preservedBuilderIds:PRESERVED_IDS,patchedInventory:true,patchedEvidence:false,installedIntake:false});
 root.__OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT_REGISTERED__=VERSION;rerenderTools();return true;
}
...
function analyzePrivescHelper(input,builderId){
 ...
 return Object.freeze({analyzer:'privesc-helper-tool-builders-current',builderId:id,cardId:CARD,outcomeFacts:facts2,state:stateFrom(states,facts2),summary:facts2.length?'Privilege-helper Evidence observed.':'No decision-relevant privilege-helper Evidence recognized yet.',redactedSample:redact(input)});
}
function patchEvidence(){
 ...
 IDS.forEach((id,i)=>{p[id]=Object.freeze({builderId:id,tools:Object.freeze([TOOLS[i]]),pathCardId:CARD,decisionStates:Object.freeze(['process observation','permission lead','exploit reference','no-hit','blocked/failure','partial','cleanup'])});});
 const profiles=Object.freeze(p),prev=typeof current.analyzeForBuilder==='function'?current.analyzeForBuilder.bind(current):()=>null;
 const patched=Object.freeze(Object.assign({},current,{version:String(current.version||'')+'+privesc-helpers-'+VERSION,profiles,analyzePrivescHelper,detectPrivescHelper:function(input){return !!helperBuilderFromText(input);},analyzeForBuilder:function(builderId,input){if(IDS.includes(builderId))return analyzePrivescHelper(input,builderId);return prev(builderId,input);},validateProfiles:function(){const failures=typeof current.validateProfiles==='function'?current.validateProfiles().slice():[];IDS.forEach(id=>{if(!profiles[id])failures.push(id+' missing privilege helper Evidence profile');});return failures;}}));
 root.OBOL_TOOL_BUILDER_EVIDENCE_CURRENT=patched;const api=root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT||{};root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT=Object.freeze(Object.assign({},api,{version:VERSION,patchedEvidence:true}));return true;
}
function installIntake(){
 const intake=root.OBOL_INTAKE_V21;if(!intake||typeof intake.analyzeTerminal!=='function'||intake.__obolPrivescHelperToolBuildersCurrent)return false;
 const prev=intake.analyzeTerminal.bind(intake);
 intake.analyzeTerminal=function(text){const base=prev(text)||{},builderId=helperBuilderFromText(text);if(!builderId)return base;const analysis=analyzePrivescHelper(text,builderId);if(!analysis.outcomeFacts.length)return base;const activities=Array.isArray(base.activities)?base.activities.slice():[];activities.push({cardId:CARD,cardIds:[CARD],kind:'tool-builder-evidence',tool:TOOLS[IDS.indexOf(builderId)]||'privilege helper',builderId,title:'Privilege-helper Evidence',summary:analysis.summary,state:analysis.state,outcomeFacts:analysis.outcomeFacts.slice(),analyzer:analysis.analyzer});return Object.assign({},base,{activities});};
 intake.__obolPrivescHelperToolBuildersCurrent=true;const api=root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT||{};root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT=Object.freeze(Object.assign({},api,{version:VERSION,installedIntake:true}));return true;
}
function install(attempt){
 ...
 const api=Object.freeze(Object.assign({},root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT||{},{version:VERSION,owner:'privesc-helper-tool-builders-current',tools:TOOLS,builderIds:IDS,preservedTools:PRESERVED_TOOLS,preservedBuilderIds:PRESERVED_IDS,installedBuilder,patchedEvidence,installedIntake,analyzePrivescHelper,detectPrivescHelper:helperBuilderFromText,rerenderTools}));
 root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT=api;
