...
const REMOTE_SRC='data/product-hardening/remote-exec-tool-builders-current.js';
...
function safeRegister(schema,item){if(!item)return null;let existing=null;if(schema.get){try{existing=schema.get(item.id);}catch(_err){existing=null;}if(existing)return existing;}try{return schema.register(item);}catch(err){if(/Duplicate Tool Builder id/i.test(String(err&&err.message||err))){if(schema.get){try{existing=schema.get(item.id);}catch(_err){existing=null;}}return existing||item;}throw err;}}
...
function loadRemoteExecOwner(){
 if(root.OBOL_REMOTE_EXEC_TOOL_BUILDERS_CURRENT||root.__OBOL_REMOTE_EXEC_TOOL_BUILDERS_CURRENT_REGISTERED__)return false;
 if(typeof document==='undefined'||typeof document.createElement!=='function')return false;
 if(document.querySelector&&document.querySelector('script[data-obol-extension="'+REMOTE_SRC+'"],script[data-obol-dashboard-src="'+REMOTE_SRC+'"]'))return false;
 const script=document.createElement('script');script.src=REMOTE_SRC;script.async=false;script.dataset.obolExtension=REMOTE_SRC;script.onload=function(){rerenderTools();};script.onerror=function(){root.__OBOL_REMOTE_EXEC_TOOL_BUILDERS_LOAD_ERROR__=REMOTE_SRC;};(document.head||document.documentElement).appendChild(script);return true;
}
function registerBuilders(){const schema=root.OBOL_TOOL_BUILDER_SCHEMA;if(!schema||typeof schema.register!=='function')return false;if(root.__OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT_REGISTERED__){patchInventory();loadRemoteExecOwner();return true;}const builders=[];for(const item of builderDefs()){const registered=safeRegister(schema,item);if(registered)builders.push(registered);}patchInventory();root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT=Object.freeze({version:VERSION,builders:Object.freeze(builders),tools:TOOLS,builderIds:IDS,preservedTools:PRESERVED_TOOLS,preservedBuilderIds:PRESERVED_IDS,patchedInventory:true,patchedEvidence:false,installedIntake:false,remoteExecOwnerSource:REMOTE_SRC});root.__OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT_REGISTERED__=VERSION;patchEvidence();loadRemoteExecOwner();rerenderTools();return true;}
...
function patchEvidence(){
 ...
 const p=Object.assign({},current.profiles||{});IDS.forEach((id,i)=>{p[id]=Object.freeze({builderId:id,tools:Object.freeze([TOOLS[i]]),pathCardId:CARD,decisionStates:Object.freeze(['process observation','permission lead','exploit reference','no-hit','blocked/failure','partial','cleanup'])});});
 const profiles=Object.freeze(p),prev=typeof current.analyzeForBuilder==='function'?current.analyzeForBuilder.bind(current):()=>null,prevValidate=typeof current.validateProfiles==='function'?current.validateProfiles.bind(current):()=>[];
 const patched=Object.freeze(Object.assign({},current,{version:String(current.version||'')+'+privesc-helpers-'+VERSION,profiles,analyzePrivescHelper,detectPrivescHelper:function(input){return !!helperBuilderFromText(input);},analyzeForBuilder:function(builderId,input){if(IDS.includes(builderId))return analyzePrivescHelper(input,builderId);return prev(builderId,input);},validateProfiles:function(){const failures=arr(prevValidate()).slice();IDS.forEach(id=>{if(!profiles[id])failures.push(id+' missing privesc-helper Evidence profile');});return failures;}}));
 root.OBOL_TOOL_BUILDER_EVIDENCE_CURRENT=patched;const owner=root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT;if(owner)root.OBOL_PRIVESC_HELPER_TOOL_BUILDERS_CURRENT=Object.freeze(Object.assign({},owner,{patchedEvidence:true}));return true;
}
function installIntake(){const intake=root.OBOL_INTAKE_V21;if(!intake||typeof intake.analyzeTerminal!=='function'||intake.__privescHelperToolBuilderCurrent)return false;const prev=intake.analyzeTerminal.bind(intake);intake.analyzeTerminal=function(text){const result=prev(text)||{},id=helperBuilderFromText(text);if(!id)return result;const analysis=analyzePrivescHelper(text,id);if(!analysis.outcomeFacts.length)return result;const activities=Array.isArray(result.activities)?result.activities.slice():[];activities.push({type:'tool-builder-evidence',builderId:id,cardId:analysis.cardId,outcomeFacts:analysis.outcomeFacts,state:analysis.state,summary:analysis.summary,source:'privesc-helper-tool-builders-current'});return Object.assign({},result,{activities});};intake.__privescHelperToolBuilderCurrent=VERSION;return true;}
registerBuilders();patchEvidence();installIntake();
