...
const packetItemMap={'web-upload-inclusion':'notes-packet-web-upload-inclusion','xss-session':'notes-packet-xss-session','credentials-auth':'notes-packet-credentials-auth'};
...
root.OBOL_PRODUCT_HARDENING_NOTE_PROGRESS=Object.freeze({schemaVersion:'1.2.0',reviewed:Number(notes.ledger.reviewedCount||0),total:Number(notes.ledger.expectedNotes||0),completedPackets:Object.freeze(Object.keys(packetItemMap).filter(id=>notes.packetReviews&&notes.packetReviews[id]&&notes.packetReviews[id].status==='complete'))});
