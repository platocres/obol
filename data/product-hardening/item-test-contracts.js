...
const contracts={
 ...
 'tb-surface-credentials':{
  acceptance:[
   'Every implemented credential/auth/cracking builder — Hashcat, John, Hydra, Kerbrute, CeWL, crunch, hashid, name-that-hash, NetExec, secretsdump, GetNPUsers, GetUserSPNs — matches the tb-ffuf operator-surface standard: grouped all-visible fieldGroups with plain-language descriptions, per-field presets, snippets where relevant, outcome-labelled mode cards, Reading-the-output row, and an honest missing-field state. tests/run-tool-surface-contract-tests.js enforces the full standard across the family.',
   'Command generation works from real operator input, not just structurally. The placeholder scrub is touched-aware: a value the operator typed/loaded/restored (including common names like hashes.txt or a real password) reaches the command, while auto-seeded/programmatic values are still blocked so demo state cannot fake a valid command. The live Tools route seeds no fabricated lab values. tests/run-tool-builder-command-generation-tests.js compiles every builder in every mode and locks the scrub in both directions.',
   'Every builder ingests its output as Evidence: hashcat/john cracked-vs-exhausted, hashid/name-that-hash routing, CeWL/crunch generation, NetExec auth/share/roast/dump, secretsdump dumps, GetNPUsers AS-REP, and GetUserSPNs SPN/TGS output map to conservative outcome facts and a positive/negative/blocked/partial state routed to the credentials, kerberos-roast, and credential-dump Path cards, with secrets redacted. Hydra and Kerbrute remain covered by tool-builder-evidence-current.js. tests/run-tool-builder-credential-evidence-tests.js proves recognition, honest inconclusive on unrelated output, and redaction.'
  ],
  validationCommands:['node tests/run-tool-surface-contract-tests.js','node tests/run-tool-builder-command-generation-tests.js','node tests/run-tool-builder-credential-evidence-tests.js','node tests/run-tool-credential-auth-guidance-tests.js','node tools/run-historical-contracts.js --phase v9-current-product','node tests/run-v10.23-tests.js'],
  proofFiles:['data/tool-builders.js','data/tool-builders-auth-enum-current.js','data/product-hardening/credential-auth-guidance-current.js','assets/tool-builder-current.js','assets/tools-library-current.js','assets/tool-builder-credential-evidence-current.js','assets/runtime-current.js','tests/run-tool-builder-command-generation-tests.js','tests/run-tool-builder-credential-evidence-tests.js','tests/run-tool-surface-contract-tests.js','docs/TOOL-BUILDER-SURFACE-STANDARD.md','docs/v10.23.md']
 },
 'tb-surface-ad-smb':{
  acceptance:[
   'Every implemented AD/SMB/remote-access builder — smbclient, smbmap, enum4linux-ng, ldapsearch, rpcclient, Responder, Evil-WinRM, Certipy, and Impacket psexec/wmiexec/smbexec/dcomexec/atexec — matches the tb-ffuf operator-surface standard: grouped all-visible fieldGroups with descriptions, per-field presets, outcome-labelled mode cards, Reading-the-output rows, and honest missing-field states. tests/run-tool-surface-contract-tests.js and tests/run-tool-ad-smb-guidance-tests.js enforce the full family.',
   'GUI controls generate real commands, not display-only affordances. Mode-card buttons map to selectable schema values, preset buttons render from schema-owned fields, toggles emit their flags, and representative AD/SMB commands include the proper options such as ldapsearch -H/-ZZ, rpcclient -U/--password/-W/-N, Responder -w/-F/-v, Certipy request flags, Evil-WinRM auth/SSL/script options, and Impacket exec artifact/output flags. tests/run-tool-builder-ad-smb-command-controls-tests.js pins this behavior.',
   'Every builder ingests its output as Evidence: share listings and permissions, enum4linux/RPC/LDAP directory facts, Responder captures, WinRM sessions, Certipy findings/cert/hash/shadow/account output, and Impacket remote-execution artifacts/output/cleanup map to conservative outcome facts and positive/negative/blocked/partial states with secrets redacted. tests/run-tool-builder-ad-smb-evidence-tests.js proves recognition, honest inconclusive on unrelated output, route-light intake installation, and redaction.'
  ],
  validationCommands:['node tests/run-tool-surface-contract-tests.js','node tests/run-tool-builder-command-generation-tests.js','node tests/run-tool-builder-ad-smb-command-controls-tests.js','node tests/run-tool-builder-ad-smb-evidence-tests.js','node tests/run-tool-ad-smb-guidance-tests.js','node tests/run-tool-builder-implemented-audit-ledger-tests.js','node tools/run-historical-contracts.js --phase v9-current-product','node tests/run-v10.24-tests.js'],
  proofFiles:['data/product-hardening/ad-smb-remote-guidance-current.js','data/product-hardening/remote-exec-tool-builders-current.js','data/tool-builders-auth-enum-current.js','data/product-hardening/tool-builder-discovery-current.js','data/tool-builders.js','data/product-hardening/tool-builder-implemented-audit-current.js','data/current-release.js','assets/runtime-current.js','tests/run-tool-builder-ad-smb-command-controls-tests.js','tests/run-tool-builder-ad-smb-evidence-tests.js','tests/run-tool-ad-smb-guidance-tests.js','tests/run-tool-surface-contract-tests.js','docs/TOOL-BUILDER-SURFACE-STANDARD.md','docs/v10.24.md']
 },
 'tb-ffuf':{
...
contracts['runtime-app-semantic-retirement']={
 acceptance:[
  ...
  'README, Product Build Next, Dashboard, runtime consolidation projection, architecture guidance, and historical v9.43/v9.46 milestones distinguish request flattening, first-paint protection, and current application execution ownership without rewriting earlier completion counts.',
  'v9.47 retires autonomous historical execution ownership rather than claiming a clean-room rewrite: still-required historical rendering semantics may remain compiled into the generated current owner while the versioned source files remain fixture/equivalence ledger only.'
 ],
...
contracts['runtime-app-single-paint']={
 acceptance:[
  ...
  'At the v9.46 milestone, the 43 application compatibility fragments remained exact-owned and frozen for regression; v9.46 changed visibility/boot ownership without claiming the later v9.47 execution-ownership retirement or changing v9.46 runtime consolidation fragment counts.'
 ],
