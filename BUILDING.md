...
v6.6 established the boundary between domain models and current project-status presentation. v6.8 added a stable non-versioned pointer for current consumers, and v7.4 continues through that boundary.

- `C.currentProjectModel(...)` is the preferred current projection boundary for canonical progress, source-fidelity progress, quality debt, Build Next, recent progress, and the next priority. In v7.4 it points to `C.projectModel74(...)`.
...
A methodology-expansion release must not skip priority 1 or 2 debt. Canonical gaps outrank source-fidelity work while gaps remain. Once gaps are zero, already-inventoried atomic units outrank still-unatomized broad sections because their missing requirements are known precisely. In v7.4, all 70 currently inventoried AD CS, Kerberos delegation, ACL / ACE, MITM / relay, and authenticated-source atomic units are complete, so the live queue remains in source-depth inventory/decomposition and proceeds into SCCM source paths. New source families must be atomized explicitly rather than silently extending an earlier completed denominator.
