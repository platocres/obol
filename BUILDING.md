...
- if you are pinned to a working branch and cannot create a `release/*` branch, title the PR `Release vX.Y: …` rather than `Obol vX.Y …`. `tools/validate-release-pr.js` treats a head matching `release/obol-vX.Y` **or** a title matching `^Obol vX.Y` as release intent, and then requires the head to be that release branch — so an `Obol vX.Y` title on a working branch fails CI with `release PR head must be release/obol-vX.Y`. The unconditional repository release contract (release doc, current regression suite, CHANGELOG entry, README markers and synchronization) still gates the release either way;
