...
The **North Star Dashboard** is the single in-app location for project-wide hard numbers. Its overview-first v6.6 layout remains the current presentation shell, while v7.0 completes the already-inventoried AD CS atomic source-fidelity queue, reconciles the certificate-mapping canonical parent only after all five subordinate units have terminal complete dispositions, and moves the live Build Next queue into Orange source inventory and decomposition. Detailed engineering metrics, source-fidelity accounting, and the complete Build Next queue remain available as drill-downs.
...
- **100 / 127** canonical sections fully implemented
- **27** partial
- **0** gaps
- **0** stale mappings
...
- **19 / 19** currently inventoried atomic units fidelity-complete
- **27** items in the live Build Next queue
- **0** implemented-quality repairs
- **0** mapped-delivery repairs

The **127 canonical** section denominator measures structural breadth. It is deliberately separate from source-depth and atomic source-fidelity progress. v7.0 advances `adcs.certificate-mapping` only after every currently inventoried subordinate certificate-mapping unit is terminal and fidelity-complete. Completing 19/19 currently inventoried AD CS units does **not** mean the pinned Orange source is exhausted: the frozen 34-section v6.2 source-depth denominator is preserved, with 27 broad source-inventory/decomposition items still live.

The detailed definitions, pinned provenance, denominators, completion language, and audit requirements live in [`docs/NORTH-STAR.md`](docs/NORTH-STAR.md). The current presentation boundary is the stable `C.currentProjectModel(...)` pointer, backed by `C.projectModel70(...)` in v7.0.
...
Obol grew through additive release overlays while the product model was still changing. Those historical layers remain where they still encode behavior protected by regression tests. v6.6 established the consolidation boundary, v6.8 added stable current-project pointers, and v7.0 continues using those pointers while moving project work from completed AD CS atomic fidelity into broader source inventory:
...
- Preserve the completed 19-unit AD CS fidelity ledger as a historical, regression-protected denominator while new source families are inventoried.
...
- **v6.8** - delivered ESC4 writable-template abuse plus both inventoried ESC7 CA-management branches with explicit reversible-change and multi-stage Evidence boundaries, reconciled the fully exhausted template/ACL canonical parents, advanced atomic source fidelity to 11/19, and added a stable `C.currentProjectModel(...)` pointer.
...
**Current live queue:** 27 items — 0 implemented-quality repairs, 0 mapped-delivery repairs, 0 canonical gaps.
**Canonical methodology:** 100/127 fully implemented (79%), 27 partial, 0 gaps, 100% represented.
**Orange source fidelity:** 1/17 source files atomized, 7/34 partial baselines decomposed, 19/19 inventoried atomic units fidelity-complete.
**Current phase:** Orange source inventory and decomposition.

**Highest-priority live items:**
1. **Constrained delegation** — delegation.md · source depth inventory.
2. **Unconstrained delegation** — delegation.md · source depth inventory.
3. **Computer object control / RBCD** — acl.md · source depth inventory.
