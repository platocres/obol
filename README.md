...
The **North Star Dashboard** is the single in-app location for project-wide hard numbers. v8.2 completes the whole-file inventory for `crack_hash.md`, advances file-level source inventory to 11/17, expands the fidelity ledger to 226/226 complete atomic units, and moves the live Build Next queue to the six remaining methodology-bearing Orange files.
...
- **100%** represented
- **11 / 17** methodology source files atomized
- **34 / 34** frozen partial baselines decomposed
- **226 / 226** currently inventoried atomic units fidelity-complete
- **6** items in the live Build Next queue
- **0** implemented-quality repairs
- **0** mapped-delivery repairs

The **127 canonical** section denominator measures structural breadth. v8.0 closed canonical breadth and the frozen 34-row v6.2 source-depth baseline. v8.1 fully atomized `low_access.md`; v8.2 fully atomizes `crack_hash.md`. Six methodology-bearing source files are still not fully atomized, so new atomic denominators will continue to appear as those files are mined end to end.

v8.2 reviews fifteen remaining terminal `crack_hash.md` variants and preserves the already-audited SCCM/PXE unit from v8.0, yielding sixteen fully accounted source elements for the file. Fourteen new units are modeled locally and the external `crack.sh` branch is explicitly superseded as the preferred path. The pinned NetNTLMv1 Hashcat `-m 1000` line is preserved in lineage but corrected on the operator surface to mode **5500** rather than copied blindly.
...
Obol grew through additive release overlays while the product model was still changing. Those historical layers remain where they still encode behavior protected by regression tests. v6.6 established the consolidation boundary, v6.8 added stable current-project pointers, and v8.2 keeps all current status consumers on those stable boundaries:
...
- Atomize the six remaining methodology-bearing source files completely before claiming Orange source inventory is complete.
- Continue through `low_hanging.md`, `persistence.md`, `dom_admin.md`, `know_vuln_auth.md`, `trusts.md`, and `valid_user.md`.
...
**Current live queue:** 6 items — 0 implemented-quality repairs, 0 mapped-delivery repairs, 0 canonical gaps.
**Canonical methodology:** 127/127 fully implemented (100%), 0 partial, 0 gaps, 100% represented.
**Orange source fidelity:** 11/17 source files atomized, 34/34 partial baselines decomposed, 226/226 inventoried atomic units fidelity-complete.
**Current phase:** Orange whole-file source inventory.

**Highest-priority live items:**
1. **Quick compromise / low-hanging fruit source inventory** — low_hanging.md · source file inventory.
2. **Domain persistence source inventory** — persistence.md · source file inventory.
3. **Domain admin source inventory** — dom_admin.md · source file inventory.
