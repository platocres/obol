...
The **North Star Dashboard** is the single in-app location for project-wide hard numbers. Its overview-first v6.6 layout remains the current presentation shell, while v7.6 atomizes the pinned `admin.md` source family into twenty-five source-fidelity units, explicitly supersedes one time-bounded legacy PPLdump route, advances the remaining frozen `admin.misc` parent, deepens the already-complete LSASS, SAM, LSA, DPAPI, and impersonation parents, and moves the live Build Next queue into `no_creds.md` source inventory. Detailed engineering metrics, source-fidelity accounting, and the complete Build Next queue remain available as drill-downs.
...
- **118 / 127** canonical sections fully implemented
- **9** partial
- **0** gaps
- **0** stale mappings
- **93%** fully implemented
- **100%** represented
- **7 / 17** methodology source files atomized
- **25 / 34** frozen partial baselines decomposed
- **118 / 118** currently inventoried atomic units fidelity-complete
- **9** items in the live Build Next queue
- **0** implemented-quality repairs
- **0** mapped-delivery repairs

The **127 canonical** section denominator measures structural breadth. It is deliberately separate from source-depth and atomic source-fidelity progress. v7.6 advances only `admin.misc`, the one `admin.md` parent that remained partial at the frozen v6.2 source-depth boundary. `admin.lsass`, `admin.sam`, `admin.lsa`, `admin.dpapi`, and `admin.impersonation` retain their historical canonical completion while gaining deeper atomic source accounting. Completing 118/118 currently inventoried units does **not** mean the pinned Orange source is exhausted: the frozen 34-section v6.2 source-depth denominator is preserved, with 9 broad source-inventory/decomposition items still live.

The detailed definitions, pinned provenance, denominators, completion language, and audit requirements live in [`docs/NORTH-STAR.md`](docs/NORTH-STAR.md). The current presentation boundary is the stable `C.currentProjectModel(...)` pointer, backed by `C.projectModel76(...)` in v7.6.
...
Obol grew through additive release overlays while the product model was still changing. Those historical layers remain where they still encode behavior protected by regression tests. v6.6 established the consolidation boundary, v6.8 added stable current-project pointers, and v7.6 continues using those pointers while extending source accounting through AD CS, Kerberos delegation, ACL / ACE control paths, MITM / relay paths, authenticated mapping, SCCM, and admin access:
...
**Current live queue:** 9 items — 0 implemented-quality repairs, 0 mapped-delivery repairs, 0 canonical gaps.
**Canonical methodology:** 118/127 fully implemented (93%), 9 partial, 0 gaps, 100% represented.
**Orange source fidelity:** 7/17 source files atomized, 25/34 partial baselines decomposed, 118/118 inventoried atomic units fidelity-complete.
**Current phase:** Orange source inventory and decomposition.

**Highest-priority live items:**
1. **Poisoning** — no_creds.md · source depth inventory.
2. **Unauthenticated coercion** — no_creds.md · source depth inventory.
3. **Certificate-based movement** — lat_move.md · source depth inventory.
