...
The **North Star Dashboard** is the single in-app location for project-wide hard numbers. v8.3 completes the whole-file inventory for `low_hanging.md`, advances file-level source inventory to 12/17, expands the fidelity ledger to 243/243 complete atomic units, and moves the live Build Next queue to the five remaining methodology-bearing Orange files.
...
- **100%** represented
- **12 / 17** methodology source files atomized
- **34 / 34** frozen partial baselines decomposed
- **243 / 243** currently inventoried atomic units fidelity-complete
- **5** items in the live Build Next queue
- **0** implemented-quality repairs
- **0** mapped-delivery repairs

The **127 canonical** section denominator measures structural breadth. v8.0 closed canonical breadth and the frozen 34-row v6.2 source-depth baseline. v8.1 fully atomized `low_access.md`; v8.2 fully atomized `crack_hash.md`; v8.3 fully atomizes `low_hanging.md`. Five methodology-bearing source files are still not fully atomized, so new atomic denominators will continue to appear as those files are mined end to end.

v8.3 reviews seventeen remaining terminal `low_hanging.md` variants and preserves the two already-audited Zerologon units from v8.0, yielding nineteen fully accounted source elements for the file. Sixteen new units are modeled through mature or focused owners. Nessus remains visible in source lineage but is explicitly superseded as Obol's default broad weak-service scanner in favor of the reproducible Nuclei plus targeted-validation workflow. Product exposure, vulnerability condition, payload delivery, callback, credential recovery, authenticated access, execution, and privilege remain separate proof states.
...
Obol grew through additive release overlays while the product model was still changing. Those historical layers remain where they still encode behavior protected by regression tests. v6.6 established the consolidation boundary, v6.8 added stable current-project pointers, and v8.3 keeps all current status consumers on those stable boundaries:
...
- Atomize the five remaining methodology-bearing source files completely before claiming Orange source inventory is complete.
- Continue through `persistence.md`, `dom_admin.md`, `know_vuln_auth.md`, `trusts.md`, and `valid_user.md`.
...
**Orange source fidelity:** 12/17 source files atomized, 34/34 partial baselines decomposed, 243/243 inventoried atomic units fidelity-complete.
**Current phase:** Orange whole-file source inventory.

**Highest-priority live items:**
1. **Domain persistence source inventory** — persistence.md · source file inventory.
2. **Domain admin source inventory** — dom_admin.md · source file inventory.
3. **Authenticated known vulnerabilities source inventory** — know_vuln_auth.md · source file inventory.
