...
1. **Post-mining modeled tool builder implementation backlog** — The implemented-builder audit and the completed web, Burp, and network/host discovery slices are no longer pending. Remaining modeled inventory records still need schema-driven builders or explicit supersession/rejection with the full Tool Builder contract before this backlog can close.
