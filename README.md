...
The **North Star Dashboard** is the single in-app location for project-wide hard numbers. Its overview-first v6.6 layout remains the current presentation shell, while v7.3 atomizes the pinned MITM / relay source family, completes ten MITM fidelity units, advances the one MITM parent that was still frozen partial at the v6.2 boundary, and continues the live Build Next queue into authenticated source inventory. Detailed engineering metrics, source-fidelity accounting, and the complete Build Next queue remain available as drill-downs.
...
- **108 / 127** canonical sections fully implemented
- **19** partial
- **0** gaps
- **0** stale mappings
...
- **15 / 34** frozen partial baselines decomposed
- **51 / 51** currently inventoried atomic units fidelity-complete
- **19** items in the live Build Next queue
- **0** implemented-quality repairs
- **0** mapped-delivery repairs

The **127 canonical** section denominator measures structural breadth. It is deliberately separate from source-depth and atomic source-fidelity progress. v7.3 advances only `mitm.listen`, which was still partial at the frozen v6.2 source-depth boundary, after the pinned `mitm.md` family is decomposed and its meaningful subordinate source units are terminal and fidelity-complete. `mitm.ntlm-relay` was already a canonical completion from v5.6 and `mitm.kerberos-relay` was already complete from v6.0; v7.3 preserves both historical milestones while adding deeper atomic source accounting. The pinned MS08-068 self-relay branch is preserved as an explicit superseded source unit rather than being silently dropped or promoted as a preferred modern workflow. Completing 51/51 currently inventoried units does **not** mean the pinned Orange source is exhausted: the frozen 34-section v6.2 source-depth denominator is preserved, with 19 broad source-inventory/decomposition items still live.

The detailed definitions, pinned provenance, denominators, completion language, and audit requirements live in [`docs/NORTH-STAR.md`](docs/NORTH-STAR.md). The current presentation boundary is the stable `C.currentProjectModel(...)` pointer, backed by `C.projectModel73(...)` in v7.3.
...
Obol grew through additive release overlays while the product model was still changing. Those historical layers remain where they still encode behavior protected by regression tests. v6.6 established the consolidation boundary, v6.8 added stable current-project pointers, and v7.3 continues using those pointers while extending source accounting through AD CS, Kerberos delegation, ACL / ACE control paths, and MITM / relay paths:
...
- **v7.1** - atomized the pinned `delegation.md` family into six meaningful units, completed discovery, unconstrained delegation, both constrained-delegation variants, RBCD, and S4U2Self fidelity, and advanced the two remaining partial delegation parents.
...
**Current live queue:** 19 items — 0 implemented-quality repairs, 0 mapped-delivery repairs, 0 canonical gaps.
**Canonical methodology:** 108/127 fully implemented (85%), 19 partial, 0 gaps, 100% represented.
**Orange source fidelity:** 4/17 source files atomized, 15/34 partial baselines decomposed, 51/51 inventoried atomic units fidelity-complete.
**Current phase:** Orange source inventory and decomposition.

**Highest-priority live items:**
1. **Authenticated coercion** — authenticated.md · source depth inventory.
2. **Authenticated known-vulnerability path** — authenticated.md · source depth inventory.
3. **Automated AD posture scans** — authenticated.md · source depth inventory.
